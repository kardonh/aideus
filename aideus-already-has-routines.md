# Aideus Agent Has Had "Routines" Since March
...
Aideus equivalent — works today:
```bash
aideus webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Aideus implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Aideus Agent |
|---|---|---|
...
## Things Aideus Does That Routines Can't
...
Aideus has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Aideus Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install aideus-agent
aideus setup
```
...
```bash
aideus gateway setup    # enable webhooks
aideus webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [aideus-agent.nousresearch.com/docs/guides/automation-templates](https://aideus-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [aideus-agent.nousresearch.com](https://aideus-agent.nousresearch.com)

GitHub: [github.com/Kardonh/aideus](https://github.com/Kardonh/aideus)

---

*Aideus Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
