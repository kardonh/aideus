...
# 5. Symlinks the 'aideus' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Aideus Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink aideus into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up aideus command..."

AIDEUS_BIN="$SCRIPT_DIR/venv/bin/aideus"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$AIDEUS_BIN" "$COMMAND_LINK_DIR/aideus"
echo -e "${GREEN}✓${NC} Symlinked aideus → $COMMAND_LINK_DISPLAY_DIR/aideus"
...
# ============================================================================
# Seed bundled skills into ~/.aideus/skills/
# ============================================================================

AIDEUS_SKILLS_DIR="${AIDEUS_HOME:-$HOME/.aideus}/skills"
mkdir -p "$AIDEUS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.aideus/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$AIDEUS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  aideus status        # Check configuration"
if is_termux; then
    echo "  aideus gateway       # Run gateway in foreground"
else
    echo "  aideus gateway install # Install gateway service (messaging + cron)"
fi
echo "  aideus cron list     # View scheduled jobs"
echo "  aideus doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m aideus_cli.main setup
fi
