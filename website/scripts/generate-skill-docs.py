...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Kardonh/aideus/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    aideus_meta = (fm.get("metadata") or {}).get("aideus") or {}
    tags = aideus_meta.get("tags") or []
    related = aideus_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `aideus skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Aideus Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Aideus ships with a large built-in skill library copied into `~/.aideus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
