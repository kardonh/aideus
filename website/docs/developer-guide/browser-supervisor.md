...
[PR #12550](https://github.com/Kardonh/aideus/pull/12550) proposed a
...
(`aideus-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Aideus `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `aideus-acp`, `aideus-api-server`, core toolsets (gated on CDP reachability)
...
- `aideus_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
