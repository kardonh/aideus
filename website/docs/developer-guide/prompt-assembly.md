...
description: "How Aideus builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Aideus deliberately separates:
...
1. agent identity — `SOUL.md` from `AIDEUS_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.aideus/SOUL.md)
You are Aideus, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.aideus/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_aideus_md(cwd_path)       # 1. .aideus.md / AIDEUS.md (walks to git root)
    ...
    # SOUL.md from AIDEUS_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.aideus.md`, `AIDEUS.md` | CWD up to git root | Aideus-native project config |
...
- **YAML frontmatter stripped** — `.aideus.md` frontmatter is removed (reserved for future config overrides)
...
1. `.aideus.md` / `AIDEUS.md` (walks to git root)
