...
description: "How Aideus stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `aideus_cli/cron.py` | CLI `aideus cron` subcommands |
...
Jobs are stored in `~/.aideus/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `aideus cron` commands are run or during active CLI sessions.
...
```python
# ~/.aideus/scripts/check_competitors.py
...
2. **Environment variable** — `AIDEUS_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.aideus/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `aideus cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `aideus cron` CLI provides direct job management:

```bash
aideus cron list                    # Show all jobs
aideus cron create                  # Interactive job creation (alias: add)
aideus cron edit <job_id>           # Edit job configuration
aideus cron pause <job_id>          # Pause a running job
aideus cron resume <job_id>         # Resume a paused job
aideus cron run <job_id>            # Trigger immediate execution
aideus cron remove <job_id>         # Delete a job
```
