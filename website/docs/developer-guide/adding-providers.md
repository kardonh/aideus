...
description: "How to add a new inference provider to Aideus Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Aideus can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `aideus model` menu entries
...
1. `aideus_cli/auth.py` decides how credentials are found.
2. `aideus_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `aideus_cli/models.py` and `aideus_cli/main.py` make the provider show up in the CLI. (`aideus_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `aideus_cli/auth.py`
2. `aideus_cli/models.py`
3. `aideus_cli/runtime_provider.py`
4. `aideus_cli/main.py`
...
:::tip
`aideus_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `aideus setup`.
:::
...
- `PROVIDER_REGISTRY` in `aideus_cli/auth.py`
- `_PROVIDER_LABELS` in `aideus_cli/models.py`
- `_PROVIDER_ALIASES` in both `aideus_cli/auth.py` and `aideus_cli/models.py`
- CLI `--provider` choices in `aideus_cli/main.py`
...
## Step 2: Add auth metadata in `aideus_cli/auth.py`
...
- What env vars should Aideus check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `aideus_cli/models.py`
...
## Step 4: Resolve runtime data in `aideus_cli/runtime_provider.py`
...
Be careful with API-key precedence. Aideus already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `aideus_cli/main.py`

A provider is not discoverable until it shows up in the interactive `aideus model` flow.

Update these in `aideus_cli/main.py`:
...
:::tip
`aideus_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `aideus model` and `aideus setup` automatically.
:::
...
When you add a native provider, double-check that Aideus is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m aideus_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m aideus_cli.main model
...
- [ ] `ProviderConfig` added in `aideus_cli/auth.py`
- [ ] aliases added in `aideus_cli/auth.py` and `aideus_cli/models.py`
- [ ] model catalog added in `aideus_cli/models.py`
- [ ] runtime branch added in `aideus_cli/runtime_provider.py`
- [ ] CLI wiring added in `aideus_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `aideus model` but not `aideus setup`
