...
sidebar_position: 1
title: "Architecture"
description: "Aideus Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Aideus Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ aideus_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AideusCLI — interactive terminal UI (~11,500 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── aideus_state.py           # SQLite session/state database with FTS5
├── aideus_constants.py       # AIDEUS_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `aideus` subcommands (~10,400 lines)
...
│   ├── skills_config.py      # aideus skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # aideus tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # aideus gateway start/stop
│
...
```text
User input → AideusCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .aideus.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.aideus/plugins/` (user), `.aideus/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `aideus plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-aideus-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Aideus as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`aideus -p <name>`) gets its own AIDEUS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
