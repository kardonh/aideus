...
The ACP adapter wraps Aideus' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
aideus acp / aideus-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.aideus/.env
  -> configure stderr logging
  -> construct AideusACPAgent
...
- `allow_once` -> Aideus `once`
- `allow_always` -> Aideus `always`
- reject options -> Aideus `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Aideus tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["aideus-acp"])
...
Instead it reuses Aideus' runtime resolver:

- `acp_adapter/auth.py`
- `aideus_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Aideus provider/credentials.
...
- ACP sessions are persisted to the shared `~/.aideus/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `aideus-acp` toolset definition
- `aideus_cli/main.py` — `aideus acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `aideus-acp` script
