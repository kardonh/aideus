...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Aideus Agent — schemas, handlers, registration, and toolsets"
---
...
2. **`toolsets.py`** — add tool name to `_AIDEUS_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `aideus_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `aideus_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `aideus chat -q "Use the weather tool for London"`
