...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Aideus-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Aideus Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Aideus/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- AideusAgentBaseEnv
    AideusAgentBaseEnv <|-- TerminalTestEnv
    AideusAgentBaseEnv <|-- AideusSweEnv
    AideusAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The aideus-agent layer (`environments/aideus_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls aideus-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `AideusAgentLoop` and scores the result
...
Your environment inherits from `AideusAgentBaseEnv` and implements five methods:
...
`AideusAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as aideus-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any aideus-agent tool |
...
Available parsers: `aideus`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "aideus-agent[yc-bench]"
...
```bash
python environments/aideus_swe_env/aideus_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"aideus"`, `"qwen"`, `"mistral"`)
...
```python
from environments.aideus_base_env import AideusAgentBaseEnv, AideusAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(AideusAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(AideusAgentBaseEnv):
    name = "my-env"
...
### AideusAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which aideus toolsets to enable |
...
| `tool_call_parser` | `str` | `"aideus"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "aideus-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "aideus-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── aideus_base_env.py          # Abstract base class (AideusAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (AideusAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── aideus_parser.py        # Aideus/ChatML <tool_call> format
...
├── aideus_swe_env/             # SWE-bench training environment
│
