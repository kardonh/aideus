...
sidebar_position: 7
title: "Docker"
description: "Running Aideus Agent in Docker and using Docker as a terminal backend"
---

# Aideus Agent — Docker

There are two distinct ways Docker intersects with Aideus Agent:

1. **Running Aideus IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running Aideus Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.aideus
docker run -it --rm \
  -v ~/.aideus:/opt/data \
  kardonh/aideus setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.aideus/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
```sh
docker run -d \
  --name aideus-dashboard \
  --restart unless-stopped \
  -v ~/.aideus:/opt/data \
  -p 9119:9119 \
  -e GATEWAY_HEALTH_URL=http://$HOST_IP:8642 \
  kardonh/aideus dashboard
```
...
```sh
/opt/aideus/.venv/bin/aideus
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Aideus state. It maps to your host's `~/.aideus/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Aideus configuration |
...
:::warning
Never run two Aideus **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::

## Multi-profile support

Aideus supports [multiple profiles](../reference/profile-commands.md) — separate `~/.aideus/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using Aideus' built-in multi-profile feature is not recommended.**
...
docker run -d \
  --name aideus-work \
  --restart unless-stopped \
  -v ~/.aideus-work:/opt/data \
  -p 8642:8642 \
  kardonh/aideus gateway run

# Personal profile
docker run -d \
  --name aideus-personal \
  --restart unless-stopped \
  -v ~/.aideus-personal:/opt/data \
  -p 8643:8642 \
  kardonh/aideus gateway run
```
...
- **Independent lifecycle** — upgrade, restart, pause, or roll back each agent separately (`docker restart aideus-work` leaves `aideus-personal` untouched).
...
services:
  aideus-work:
    image: kardonh/aideus:latest
    container_name: aideus-work
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
    volumes:
      - ~/.aideus-work:/opt/data

  aideus-personal:
    image: kardonh/aideus:latest
    container_name: aideus-personal
    restart: unless-stopped
    command: gateway run
    ports:
      - "8643:8642"
    volumes:
      - ~/.aideus-personal:/opt/data
```
...
services:
  ...
  dashboard:
    image: kardonh/aideus:latest
    container_name: aideus-dashboard
    restart: unless-stopped
    ...
    volumes:
      - ~/.aideus:/opt/data
    environment:
      - GATEWAY_HEALTH_URL=http://aideus:8642
    networks:
      - aideus-net
    depends_on:
      - aideus
...
The Aideus container needs moderate resources. Recommended minimums:
...
- Python 3 with all Aideus dependencies (`uv pip install -e ".[all]"`)
...
- Then runs `aideus` with whatever arguments you pass
...
```sh
docker pull kardonh/aideus:latest
docker rm -f aideus
docker run -d \
  --name aideus \
  --restart unless-stopped \
...
Check logs: `docker logs aideus`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container's entrypoint drops privileges to the non-root `aideus` user (UID 10000) via `gosu`. If your host `~/.aideus/` is owned by a different UID, set `AIDEUS_UID`/`AIDEUS_GID` to match your host user, or ensure the data directory is writable:
...
```sh
docker restart aideus
```

### Checking container health

```sh
docker logs --tail 50 aideus          # Recent logs
docker run -it --rm kardonh/aideus:latest version     # Verify version
docker stats aideus                    # Resource usage
```
