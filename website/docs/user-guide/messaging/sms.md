...
title: "SMS (Twilio)"
description: "Set up Aideus Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Aideus connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `pip install 'aideus-agent[sms]'`
...
If you're running Aideus locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Aideus will respond via SMS.
...
Aideus validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Aideus gateway logs for Twilio API errors
