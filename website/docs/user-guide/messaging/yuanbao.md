...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Aideus Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Aideus to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.aideus/.env`:
...
All standard Aideus commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.aideus/logs/gateway.log`
...
4. Consider enabling verbose logging: `AIDEUS_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Aideus automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
AIDEUS_LOG_LEVEL=debug aideus gateway
```
...
```bash
aideus chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
