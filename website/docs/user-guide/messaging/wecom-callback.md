...
Connect Aideus to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Aideus supports two WeCom integration modes:
...
3. Aideus decrypts the message, queues it for the agent
...
(Use `aideus gateway start` only after `aideus gateway install` has registered the systemd/launchd service.)
