...
title: "Git Worktrees"
description: "Run multiple Aideus agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Aideus Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Aideus so each session has a clean, isolated working directory.

## Why Use Worktrees with Aideus?

Aideus treats the **current working directory** as the project root:

- CLI: the directory where you run `aideus` or `aideus chat`
...
git worktree add ../repo-feature feature/aideus-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/aideus-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Aideus there:

```bash
cd ../repo-feature

# Start Aideus in the worktree
aideus
```
...
git worktree add ../repo-experiment-a feature/aideus-a
git worktree add ../repo-experiment-b feature/aideus-b
```
...
- Works on its own branch (`feature/aideus-a` vs `feature/aideus-b`).
...
- Aideus checkpoint data under `~/.aideus/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Aideus experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/aideus-checkpoints-docs`, `feature/aideus-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Aideus from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `aideus -w` (Automatic Worktree Mode)

Aideus has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `aideus/aideus-<hash>`).
...
```bash
aideus -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `aideus -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Aideus session its own clean checkout.
