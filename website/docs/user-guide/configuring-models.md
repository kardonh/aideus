...
Aideus uses two kinds of model slots:
...
- **Right** — the curated model list for the selected provider. These are the agentic models Aideus recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Aideus writes it to `~/.aideus/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning Aideus uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `aideus skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, Aideus writes to `~/.aideus/config.yaml`:
...
`provider: auto` with `model: ''` tells Aideus to use the main model for that task.

## When does it take effect?

- **CLI** (`aideus chat`): next `aideus chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`aideus gateway restart`) if you want to force all sessions to pick up the change.
...
Aideus lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `aideus setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Aideus switched providers on me
...
Inside any `aideus chat` session:
...
### `aideus model` subcommand

```bash
aideus model list                   # list authenticated providers + models
aideus model set anthropic/claude-opus-4.7 --provider openrouter
```

### Direct config edit

Edit `~/.aideus/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-Aideus-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Aideus-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Aideus-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__AIDEUS_SESSION_TOKEN__`) if you're scripting against a running dashboard.
