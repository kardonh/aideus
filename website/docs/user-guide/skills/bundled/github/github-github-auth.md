...
- Give it a name like "aideus-agent"
- Select scopes:
...
- Give it a title like "aideus-agent-&lt;machine-name>"
...
elif [ -f ~/.aideus/.env ] && grep -q "^GITHUB_TOKEN=" ~/.aideus/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.aideus/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
