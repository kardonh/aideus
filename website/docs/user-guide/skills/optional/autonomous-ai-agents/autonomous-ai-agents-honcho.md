...
description: "Configure and use Honcho memory with Aideus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Aideus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aideus skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Aideus

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Aideus profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/aideus#running-honcho-locally-with-aideus

### Verify

```bash
aideus honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Aideus profile's AI peer
...
Honcho models conversations as interactions between **peers**. Aideus creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Aideus instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Aideus run |
| `global` | Single session across all directories |

Manual override: `aideus honcho map my-project-name`
...
Each Aideus profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block aideus.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `aideus.coder` host block in `honcho.json`
...
```bash
aideus honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Aideus when Honcho memory is active.
...
Config file: `$AIDEUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `aideus honcho setup`. Ensure `memory.provider: honcho` is in `~/.aideus/config.yaml`.

### Memory not persisting across sessions
Check `aideus honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `aideus profile create <name> --clone`. For existing profiles: `aideus honcho sync`.
...
| Command | Description |
|---------|-------------|
| `aideus honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `aideus honcho status` | Show resolved config, connection test, peer info for active profile |
| `aideus honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `aideus honcho disable` | Disable Honcho for the active profile |
| `aideus honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `aideus honcho peers` | Show peer identities across all profiles |
| `aideus honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `aideus honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `aideus honcho sessions` | List known directory-to-session-name mappings |
| `aideus honcho map <name>` | Map current working directory to a Honcho session name |
| `aideus honcho identity` | Seed AI peer identity or show both peer representations |
| `aideus honcho sync` | Create host blocks for all Aideus profiles that don't have one yet |
| `aideus honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Aideus + Honcho |
| `aideus memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `aideus memory status` | Show active memory provider and config |
| `aideus memory off` | Disable external memory provider |
