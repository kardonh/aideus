---
title: "Aideus Atropos Environments — Build, test, and debug Aideus Agent RL environments for Atropos training"
sidebar_label: "Aideus Atropos Environments"
description: "Build, test, and debug Aideus Agent RL environments for Atropos training"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Aideus Atropos Environments

Build, test, and debug Aideus Agent RL environments for Atropos training. Covers the AideusAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the aideus-agent repo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aideus skills install official/mlops/aideus-atropos-environments` |
| Path | `optional-skills/mlops/aideus-atropos-environments` |
| Version | `1.1.0` |
| Author | Aideus Agent |
| License | MIT |
...
Guide for building RL environments in the aideus-agent repo that integrate with the Atropos training framework.
...
Atropos BaseEnv (atroposlib/envs/base.py)
    └── AideusAgentBaseEnv (environments/aideus_base_env.py)
...
Aideus environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/aideus_base_env.py` | Base class with agent loop + tool resolution |
| `environments/agent_loop.py` | `AideusAgentLoop` + `AgentResult` dataclass |
| `environments/tool_context.py` | `ToolContext` for reward verification |
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (aideus, mistral, etc.) |
...
The whole point of aideus-agent environments is agentic evaluation:

```python
async def evaluate(self, *args, **kwargs) -> None:
    import time, uuid
    from environments.agent_loop import AideusAgentLoop
...
2. **evaluate() must use AideusAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of aideus-agent benchmarks is agentic evaluation with tool use.
