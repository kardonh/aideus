...
| | |
|---|---|
| Source | Optional — install with `aideus skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Aideus or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Aideus. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.aideus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.aideus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Aideus integration, either:

- configure the server in `~/.aideus/config.yaml` using the `native-mcp` skill, or
...
### Aideus cannot see the deployed server

The server-building part may be correct while the Aideus config is not. Load the `native-mcp` skill and configure the server in `~/.aideus/config.yaml`, then restart Aideus.
