...
| | |
|---|---|
| Source | Optional — install with `aideus skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Aideus core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Aideus native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Aideus native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Aideus native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Aideus usage patterns
...
- Do not silently choose Parallel when Aideus native tools are already sufficient.
