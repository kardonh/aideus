...
| | |
|---|---|
| Source | Optional — install with `aideus skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Aideus Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Aideus terminal calls (desktop app flow only)
...
### Service Account (recommended for Aideus)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.aideus/.env` (the skill will prompt for this on first load).
...
## Aideus Execution Pattern (desktop app flow)

Aideus terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/aideus-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/aideus-op.sock"
