...
Aideus Agent includes an integrated RL (Reinforcement Learning) training pipeline built on **Tinker-Atropos**. This enables training language models on environment-specific tasks using GRPO (Group Relative Policy Optimization) with LoRA adapters, orchestrated entirely through the agent's tool interface.
...
- The `tinker-atropos` submodule (at `tinker-atropos/` relative to the Aideus root)

```bash
# Set up API keys
aideus config set TINKER_API_KEY your-tinker-key
aideus config set WANDB_API_KEY your-wandb-key
```
...
Each training run generates log files in `~/.aideus/logs/rl_training/`:
