...
Aideus Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
aideus memory setup      # interactive picker + configuration
aideus memory status     # check what's active
aideus memory off        # disable external provider
```

You can also select the active memory provider via `aideus plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.aideus/config.yaml`:
...
When a memory provider is active, Aideus automatically:
...
**Setup Wizard:**
```bash
aideus honcho setup        # (legacy command) 
# or
aideus memory setup        # select "honcho"
```

**Config:** `$AIDEUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$AIDEUS_HOME/honcho.json` > `~/.aideus/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/aideus-ai/aideus-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/aideus).
...
:::tip Migrating from `aideus honcho`
If you previously used `aideus honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Aideus profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Aideus profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Aideus profile. Host key `aideus` → default; `aideus.<profile>` for others. |
...
```bash
aideus profile create coder --clone
```

`--clone` creates a `aideus.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Aideus profile, creates host blocks for any profile without one, inherits settings from the default `aideus` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/aideus-ai/aideus-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/aideus).
...
aideus memory setup    # select "openviking"
# Or manually:
aideus config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.aideus/.env
```
...
**Setup:**
```bash
aideus memory setup    # select "mem0"
# Or manually:
aideus config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.aideus/.env
```

**Config:** `$AIDEUS_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `aideus-user` | User identifier |
| `agent_id` | `aideus` | Agent identifier |
...
**Setup:**
```bash
aideus memory setup    # select "hindsight"
# Or manually:
aideus config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.aideus/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p aideus ui start`

**Config:** `$AIDEUS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `aideus` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Aideus Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Kardonh/aideus/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
aideus memory setup    # select "holographic"
# Or manually:
aideus config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.aideus-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$AIDEUS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
aideus memory setup    # select "retaindb"
# Or manually:
aideus config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.aideus/.env
```
...
# Then configure Aideus
aideus memory setup    # select "byterover"
# Or manually:
aideus config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$AIDEUS_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
aideus memory setup    # select "supermemory"
# Or manually:
aideus config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.aideus/.env
```

**Config:** `$AIDEUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `aideus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `aideus-{identity}` → `aideus-coder`) to isolate memories per Aideus profile
...
- **Local storage providers** (Holographic, ByteRover) use `$AIDEUS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$AIDEUS_HOME/` so each profile has its own credentials
