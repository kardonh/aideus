...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Aideus Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Aideus Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Aideus](/docs/guides/use-voice-mode-with-aideus).

## Prerequisites

Before using voice features, make sure you have:

1. **Aideus Agent installed** — `pip install aideus-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `aideus model` or set your preferred provider credentials in `~/.aideus/.env`
3. **A working base setup** — run `aideus` to verify the agent responds to text before enabling voice

:::tip
The `~/.aideus/` directory and default `config.yaml` are created automatically the first time you run `aideus`. You only need to create `~/.aideus/.env` manually for API keys.
:::
...
pip install "aideus-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "aideus-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`aideus chat`) and the **TUI** (`aideus --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.aideus/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
aideus                # Start the interactive CLI
```
...
1. Start the CLI with `aideus` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.aideus/config.yaml` (default: `ctrl+b`).
:::
...
```bash
aideus gateway        # Start the gateway (connects to configured platforms)
aideus gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@aideusbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.aideus/.env`:
```bash
...
```bash
aideus gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.aideus/.env`
...
- Review gateway logs: `tail -f ~/.aideus/logs/gateway.log`
