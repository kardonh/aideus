...
Type `@` followed by a reference to inject content directly into your message. Aideus expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Aideus env: `$AIDEUS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$AIDEUS_HOME/skills/.hub/`
