...
description: "Customize Aideus Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Aideus Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `AIDEUS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Aideus is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Aideus now seeds a default `SOUL.md` automatically in:

```text
~/.aideus/SOUL.md
```

More precisely, it uses the current instance's `AIDEUS_HOME`, so if you run Aideus with a custom home directory, it will use:
...
- Aideus creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Aideus loads `SOUL.md` only from `AIDEUS_HOME`
- Aideus does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Aideus falls back to a built-in default identity
...
If Aideus loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `AIDEUS_HOME`, the personality belongs to the Aideus instance itself.

That also makes it easier to teach users:
- "Edit `~/.aideus/SOUL.md` to change Aideus' default personality."
...
- how Aideus should handle uncertainty, disagreement, or ambiguity
...
## What Aideus injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Aideus falls back to a built-in default identity ("You are Aideus Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Aideus ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Aideus, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Aideus its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.aideus/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.aideus/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-aideus)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Aideus speaks
- `display.skin` and `/skin` affect how Aideus looks in the terminal
