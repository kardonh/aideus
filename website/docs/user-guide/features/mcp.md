...
description: "Connect Aideus Agent to external tool servers via MCP — and control exactly which MCP tools Aideus loads"
---

# MCP (Model Context Protocol)

MCP lets Aideus Agent connect to external tool servers so the agent can use tools that live outside Aideus itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Aideus to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Aideus tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Aideus to see
...
```bash
cd ~/.aideus/aideus-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.aideus/config.yaml`:
...
4. Ask Aideus to use the MCP-backed capability.
...
Aideus will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Aideus connects to directly.
...
- you do not want Aideus spawning a local subprocess for that integration

## Basic configuration reference

Aideus reads MCP config from `~/.aideus/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Aideus skips the server entirely |
...
## How Aideus registers MCP tools

Aideus prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Aideus sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Aideus also registers utility tools around MCP resources and prompts:
...
- Aideus only registers resource utilities if the MCP session actually supports resource operations
- Aideus only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Aideus, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Aideus skips the server completely and does not even attempt a connection.
...
You can also separately disable Aideus-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Aideus does not create an empty runtime MCP toolset for that server.
...
Aideus discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Aideus when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Aideus receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Aideus does not blindly pass your full shell environment.
...
Then verify your config and restart Aideus.
...
Because Aideus now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Aideus via the `sampling/createMessage` protocol. This allows an MCP server to ask Aideus to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Aideus as an MCP server

In addition to connecting **to** MCP servers, Aideus can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Aideus's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Aideus
- You want a single MCP server that bridges to all of Aideus's connected messaging platforms at once
- You already have a running Aideus gateway with connected platforms
...
Add Aideus to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Aideus in a specific location:

```json
{
  "mcpServers": {
    "aideus": {
      "command": "/home/user/.aideus/aideus-agent/venv/bin/aideus",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Aideus-specific channel browser:
...
The MCP server includes a live event bridge that polls Aideus's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
aideus mcp serve              # Normal mode
aideus mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Aideus's session store (`~/.aideus/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Aideus agent itself.
