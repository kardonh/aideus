...
Schedule tasks to run automatically with natural language or cron expressions. Aideus exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Aideus disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
aideus cron create "every 2h" "Check server status"
aideus cron create "every 1h" "Summarize new feed items" --skill blogwatcher
aideus cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Aideus will use the unified `cronjob` tool internally.
...
aideus cron create "every 1d at 09:00" \
...
```bash
aideus cron edit <job_id> --schedule "every 4h"
aideus cron edit <job_id> --prompt "Use the revised task"
aideus cron edit <job_id> --skill blogwatcher --skill maps
aideus cron edit <job_id> --add-skill maps
aideus cron edit <job_id> --remove-skill blogwatcher
aideus cron edit <job_id> --clear-skills
```
...
```bash
aideus cron list
aideus cron pause <job_id>
aideus cron resume <job_id>
aideus cron run <job_id>
aideus cron remove <job_id>
aideus cron status
aideus cron tick
```
...
sudo aideus gateway install --system   # Linux: boot-time system service for servers
aideus gateway             # Or run in foreground
...
1. loads jobs from `~/.aideus/cron/jobs.json`
...
A file lock at `~/.aideus/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.aideus/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.aideus/cron/output/`), but no message is sent to the delivery target.
...
Or set the `AIDEUS_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Aideus skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `aideus tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `aideus tools` cron-platform config wins; otherwise Aideus falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Aideus should even invoke the agent. Emit a final stdout line of the form:
...
Jobs are stored in `~/.aideus/cron/jobs.json`. Output from job runs is saved to `~/.aideus/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Aideus resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
