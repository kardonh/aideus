...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Aideus's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Aideus instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
aideus memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$AIDEUS_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `aideus` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Aideus syncs them back at session init.
...
```bash
aideus honcho status          # Connection status, config, and key settings
aideus honcho setup           # Interactive setup wizard
aideus honcho strategy        # Show or set session strategy
aideus honcho peer            # Update peer names for multi-agent setups
aideus honcho mode            # Show or set recall mode
aideus honcho tokens          # Show or set context token budget
aideus honcho identity        # Show Honcho peer identity
aideus honcho sync            # Sync host blocks for all profiles
aideus honcho enable          # Enable Honcho
aideus honcho disable         # Disable Honcho
```

## Migrating from `aideus honcho`

If you previously used the standalone `aideus honcho setup`:
...
No re-login or re-setup needed. Run `aideus memory setup` and select "honcho" — the wizard detects your existing config.
