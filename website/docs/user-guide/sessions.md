...
Aideus Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.aideus/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.aideus/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`aideus` or `aideus chat`) |
...
aideus -c "my project"   # → resumes "my project #3"
```
...
aideus --resume 20250305_091523_a1b2c3d4
aideus -r 20250305_091523_a1b2c3d4

# Resume by title
aideus --resume "refactoring auth"

# Or with the chat subcommand
aideus chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `aideus sessions list`.

### Conversation Recap on Resume

When you resume a session, Aideus displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Aideus session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.aideus/config.yaml`:
...
Aideus automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `aideus sessions list` or `aideus sessions browse`.
...
```bash
aideus sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Aideus creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`aideus -c "my project"`), it automatically picks the most recent session in the lineage.
...
Aideus provides a full set of session management commands via `aideus sessions`:
...
# Filter by platform
aideus sessions list --source telegram

# Show more sessions
aideus sessions list --limit 50
```
...
aideus sessions export backup.jsonl

# Export sessions from a specific platform
aideus sessions export telegram-history.jsonl --source telegram

# Export a single session
aideus sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
aideus sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
aideus sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
aideus sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
aideus sessions prune

# Custom age threshold
aideus sessions prune --older-than 30

# Only prune sessions from a specific platform
aideus sessions prune --source telegram --older-than 60

# Skip confirmation
aideus sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`aideus insights`](/docs/reference/cli-commands#aideus-insights).
...
When Aideus cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Aideus uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Aideus in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.aideus/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.aideus/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.aideus/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Aideus process in the same `AIDEUS_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.aideus/config.yaml`:
...
aideus sessions delete <session_id>
