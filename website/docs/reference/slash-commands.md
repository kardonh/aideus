...
Aideus has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `aideus_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.aideus/plans/` relative to the active workspace/backend working directory.
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of Aideus config/state. `create [label]` saves a snapshot, `restore <id>` reverts to it, `prune [N]` removes old snapshots, or list all with no args. |
...
| `/busy [queue\|steer\|interrupt\|status]` | CLI-only: control what pressing Enter does while Aideus is working — queue the new message, steer mid-turn, or interrupt immediately. |
...
User-defined quick commands map a short slash command to either a shell command or another slash command. Configure them in `~/.aideus/config.yaml`:
