...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Aideus Agent — no API key required"
---

# MiniMax OAuth

Aideus Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Aideus automatically refreshes your session.
...
# → Aideus opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.aideus/auth.json` and are refreshed automatically before each session.
...
```bash
aideus auth add minimax-oauth --region cn
```
...
```bash
aideus auth add minimax-oauth --no-browser
```

Aideus will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Aideus implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Aideus generates a PKCE verifier / challenge pair and a random state value.
...
4. Aideus polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.aideus/auth.json` under the `minimax-oauth` key.
...
```bash
aideus config set model MiniMax-M2.7
aideus config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.aideus/config.yaml` will contain entries similar to:
...
```bash
aideus --provider minimax-oauth    # canonical
aideus --provider minimax-portal   # alias
aideus --provider minimax-global   # alias
aideus --provider minimax_oauth    # alias (underscore form)
```
...
```bash
AIDEUS_INFERENCE_PROVIDER=minimax-oauth aideus
```
...
Aideus refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Aideus marks the session as requiring re-login.

**Fix:** run `aideus auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Aideus raises a timeout error.

**Fix:** re-run `aideus auth add minimax-oauth` (or `aideus model`). The flow starts fresh.

### State mismatch (possible CSRF)

Aideus detected that the `state` value returned by the authorization server does not match what it sent.
...
If `aideus` cannot open a browser window, use `--no-browser`:

```bash
aideus auth add minimax-oauth --no-browser
```

Aideus prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `aideus model` and select MiniMax (OAuth), or run `aideus auth add minimax-oauth`.
...
```bash
aideus auth remove minimax-oauth
```
...
- [aideus doctor](../reference/cli-commands.md)
