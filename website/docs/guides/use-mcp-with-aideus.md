---
sidebar_position: 6
title: "Use MCP with Aideus"
description: "A practical guide to connecting MCP servers to Aideus Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Aideus

This guide shows how to actually use MCP with Aideus Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Aideus tool
- you want Aideus to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Aideus to internal APIs, databases, or company systems without modifying Aideus core

Do not use MCP when:
- a built-in Aideus tool already solves the job well
...
- Aideus remains the agent
- MCP servers contribute tools
- Aideus discovers those tools at startup or reload time
...
If you installed Aideus with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Aideus banner/status should show MCP integration when configured
- ask Aideus what tools it has available
...
There are two categories of MCP-exposed functionality in Aideus:
...
2. Aideus-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Aideus will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Aideus to reason over a bounded workspace.
...
Now Aideus can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Aideus core.
...
Because Aideus now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
