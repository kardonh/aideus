---
sidebar_position: 7
title: "Use SOUL.md with Aideus"
description: "How to use SOUL.md to shape Aideus Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Aideus

`SOUL.md` is the **primary identity** for your Aideus instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Aideus to feel like the same assistant every time you talk to it — or if you want to replace the Aideus persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Aideus should be
- what Aideus should avoid stylistically
- how Aideus should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Aideus is and how Aideus speaks
...
Aideus now uses only the global SOUL file for the current instance:

```text
~/.aideus/SOUL.md
```

If you run Aideus with a custom home directory, it becomes:
...
Aideus automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Aideus does not overwrite it
- if the file exists but is empty, Aideus adds nothing from it to the prompt

## How Aideus uses it

When Aideus starts a session, it reads `SOUL.md` from `AIDEUS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Aideus feels.
...
Aideus already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Aideus should sound.

# Avoid
What Aideus should not do.

# Defaults
How Aideus should behave when ambiguity appears.
```
...
Then restart Aideus or start a new session.
...
### I edited SOUL.md but Aideus still sounds the same

Check:
- you edited `~/.aideus/SOUL.md` or `$AIDEUS_HOME/SOUL.md`
...
### Aideus is ignoring parts of my SOUL.md
