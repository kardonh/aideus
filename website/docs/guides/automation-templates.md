...
Copy-paste recipes for common automation patterns. Each template uses Aideus's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`aideus webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `aideus webhook subscribe`) |
...
```bash
aideus cron create "0 2 * * *" \
  "You are a project manager triaging the Kardonh/aideus GitHub repo.

1. Run: gh issue list --repo Kardonh/aideus --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
aideus webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
aideus cron create "0 9 * * 1" \
  "Scan the Kardonh/aideus repo for documentation drift.

1. Run: gh pr list --repo Kardonh/aideus --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (aideus_cli/commands.py, aideus_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (aideus_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
aideus cron create "0 6 * * *" \
  "Run a dependency security audit on the aideus-agent project.

1. cd ~/.aideus/aideus-agent && source .venv/bin/activate
...
```bash
aideus webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
aideus webhook subscribe alert-triage \
...
```python title="~/.aideus/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Aideus-Monitor/1.0"})
...
aideus cron create "every 30m" \
  ...
  --script ~/.aideus/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
aideus cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Aideus Agent development." \
...
```bash
aideus webhook subscribe github-issues \
  --events "issues" \
...
```bash
aideus webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
aideus webhook subscribe stripe-payments \
...
```bash
aideus cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the aideus-agent codebase.
