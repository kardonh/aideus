---
sidebar_position: 8
title: "Use Voice Mode with Aideus"
description: "A practical guide to setting up and using Aideus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Aideus
...
- you want Aideus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Aideus.
...
## Step 1: make sure normal Aideus works first
...
Aideus supports both local and cloud speech stacks.
...
### If you use `aideus setup`

If you choose NeuTTS in the setup wizard, Aideus checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Aideus transcribes and responds
...
- asking Aideus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Aideus loop.

## Tuning CLI behavior

### Silence threshold

If Aideus starts/stops too aggressively, tune:
...
Aideus stays a normal chat bot, but can speak replies.
...
- you want Aideus to function like a portable research or ops assistant
...
Aideus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Aideus detects speech boundaries
- transcripts are posted in the associated text channel
- Aideus responds in text and audio
...
1. get text Aideus working
2. install `aideus-agent[voice]`
