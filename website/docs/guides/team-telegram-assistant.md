...
This tutorial walks you through setting up a Telegram bot powered by Aideus Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Aideus Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.aideus/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Aideus itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Aideus Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_aideus_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Aideus Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Aideus Gateway...
...
# View live logs
journalctl --user -u aideus-gateway -f
...
aideus gateway start
aideus gateway stop
tail -f ~/.aideus/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `aideus gateway install` to update the plist.
:::
...
```bash
aideus gateway stop && aideus gateway start
```
...
aideus pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.aideus/.env`:
...
Control how much detail the bot shows when using tools. In `~/.aideus/config.yaml`:
...
Customize how the bot communicates by editing `~/.aideus/SOUL.md`:

For a full guide, see [Use SOUL.md with Aideus](/docs/guides/use-soul-with-aideus).
...
```bash
# From the CLI
aideus cron list          # View all scheduled jobs
aideus cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u aideus-gateway` (Linux) or `~/.aideus/logs/gateway.log` (macOS) |
| Cron job output | `~/.aideus/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.aideus/cron/jobs.json` |
| Pairing data | `~/.aideus/pairing/` |
| Session history | `~/.aideus/sessions/` |
