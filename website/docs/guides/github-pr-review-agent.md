...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Aideus Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Aideus instantly when PRs are opened or updated.
:::
...
- **Aideus Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  aideus gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.aideus/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Aideus can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Kardonh/aideus --state open --limit 3
```
...
Still in the chat, ask Aideus to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Kardonh/aideus
```
...
A skill gives Aideus consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.aideus/skills/code-review
```

Create `~/.aideus/skills/code-review/SKILL.md`:
...
Verify it loaded — start `aideus` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Aideus your team's standards:
...
2. Teach Aideus your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
aideus gateway status    # Is the gateway running?
aideus cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-aideus-plugin)** — wrap the review logic into a shareable plugin
