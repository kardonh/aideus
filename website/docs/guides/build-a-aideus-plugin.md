...
title: "Build a Aideus Plugin"
description: "Step-by-step guide to building a complete Aideus plugin with tools, hooks, data files, and skills"
---

# Build a Aideus Plugin

This guide walks through building a complete Aideus plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.aideus/plugins/calculator
cd ~/.aideus/plugins/calculator
```
...
This tells Aideus: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Aideus may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `aideus my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Aideus continues fine
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.aideus/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.aideus/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `aideus plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Aideus injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: aideus-agent"}
...
- **The system prompt is Aideus's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `aideus <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for aideus my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: aideus my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for aideus my-plugin."""
...
After registration, users can run `aideus my-plugin status`, `aideus my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `aideus name` in a terminal |
...
```toml
# pyproject.toml
[project.entry-points."aideus_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install aideus-plugin-calculator
# Plugin auto-discovered on next aideus startup
```
...
```python
# Wrong — will break if Aideus passes extra context
def handler(args):
    ...
