"""Persistent slash-command worker — one AideusCLI per TUI session.
...
def _run(cli: AideusCLI, command: str) -> str:
...
def main():
    ...
    os.environ["AIDEUS_SESSION_KEY"] = args.session_key
    os.environ["AIDEUS_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AideusCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
