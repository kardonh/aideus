...
VERSION_FILE = REPO_ROOT / "aideus_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+aideus-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "aideus@nousresearch.com": "NousResearch",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "aideus@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Kardonh/aideus",
    ...
    lines.append(f"# Aideus Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Aideus Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Aideus Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Aideus Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Aideus Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Aideus Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Aideus Agent v{new_version} ({calver_date})' "
