#!/usr/bin/env python3
"""Drive the Aideus TUI under AIDEUS_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.aideus/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with AIDEUS_TUI_RESUME set so it
bypasses the aideus_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  AIDEUS_PERF_LOG     (default ~/.aideus/perf.log)
  AIDEUS_PERF_NODE    (default node from $PATH)
  AIDEUS_TUI_DIR      (default /home/bb/aideus-agent/ui-tui)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests AIDEUS_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(os.environ.get("AIDEUS_TUI_DIR", "/home/bb/aideus-agent/ui-tui"))
DEFAULT_LOG = Path(os.environ.get("AIDEUS_PERF_LOG", str(Path.home() / ".aideus" / "perf.log")))
DEFAULT_STATE_DB = Path.home() / ".aideus" / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — AIDEUS_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["AIDEUS_DEV_PERF"] = "1"
    env["AIDEUS_DEV_PERF_MS"] = str(args.threshold_ms)
    env["AIDEUS_DEV_PERF_LOG"] = str(log)
    env["AIDEUS_TUI_RESUME"] = sid
    ...
    node = os.environ.get("AIDEUS_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="AIDEUS_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "aideus-ink" / "src"
