# ============================================================================
# Aideus Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/Kardonh/aideus/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$AideusHome = "$env:LOCALAPPDATA\aideus",
    [string]$InstallDir = "$env:LOCALAPPDATA\aideus\aideus-agent"
)
...
$RepoUrlSsh = "git@github.com:Kardonh/aideus.git"
$RepoUrlHttps = "https://github.com/Kardonh/aideus.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Aideus Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$AideusHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$AideusHome\node;$env:Path"
        Write-Success "Node.js $version found (Aideus-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.aideus/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\aideus-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AideusHome\node") { Remove-Item -Recurse -Force "$AideusHome\node" }
                Move-Item $extractedDir.FullName "$AideusHome\node"
                $env:Path = "$AideusHome\node;$env:Path"

                $version = & "$AideusHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.aideus/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/Kardonh/aideus/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\aideus-agent-$Branch.zip"
                $extractPath = "$env:TEMP\aideus-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up aideus command..."
    
    if ($NoVenv) {
        $aideusBin = "$InstallDir"
    } else {
        $aideusBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so aideus is globally available
    # On Windows, the aideus.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$aideusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$aideusBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $aideusBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set AIDEUS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\aideus instead
    # of the Unix default ~/.aideus
    $currentAideusHome = [Environment]::GetEnvironmentVariable("AIDEUS_HOME", "User")
    if (-not $currentAideusHome -or $currentAideusHome -ne $AideusHome) {
        [Environment]::SetEnvironmentVariable("AIDEUS_HOME", $AideusHome, "User")
        Write-Success "Set AIDEUS_HOME=$AideusHome"
    }
    $env:AIDEUS_HOME = $AideusHome
    
    # Update current session
    $env:Path = "$aideusBin;$env:Path"
    
    Write-Success "aideus command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.aideus directory structure
    New-Item -ItemType Directory -Force -Path "$AideusHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AideusHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AideusHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.aideus/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.aideus/.env"
        }
    } else {
        Write-Info "~/.aideus/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$AideusHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.aideus/config.yaml from template"
        }
    } else {
        Write-Info "~/.aideus/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$AideusHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.aideus/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.aideus/"
    
    # Seed bundled skills into ~/.aideus/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.aideus/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.aideus/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AideusHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.aideus/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (aideus --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m aideus_cli.main setup
    } else {
        python -m aideus_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $aideusCmd = "$InstallDir\venv\Scripts\aideus.exe"
    if (-not (Test-Path $aideusCmd)) {
        $aideusCmd = "aideus"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AideusHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'aideus whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AideusHome\logs\gateway.log"
            Start-Process -FilePath $aideusCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AideusHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: aideus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: aideus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AideusHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AideusHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AideusHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AideusHome\aideus-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   aideus              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   aideus setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   aideus config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   aideus config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   aideus gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   aideus update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Kardonh/aideus/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
