"""CLI entry point for the aideus-agent ACP adapter.

Loads environment variables from ``~/.aideus/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from AIDEUS_HOME (default ``~/.aideus``)."""
    from aideus_cli.env_loader import load_aideus_dotenv

    aideus_home = get_aideus_home()
    loaded = load_aideus_dotenv(aideus_home=aideus_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", aideus_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting aideus-agent ACP adapter")
    ...
    import acp
    from .server import AideusACPAgent
