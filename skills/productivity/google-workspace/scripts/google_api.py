#!/usr/bin/env python3
"""Google Workspace API CLI for Aideus Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Aideus-facing JSON contract and falls back to the Python client
...
from _aideus_home import get_aideus_home
...
def _gws_binary() -> str | None:
    override = os.getenv("AIDEUS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Aideus Agent")
