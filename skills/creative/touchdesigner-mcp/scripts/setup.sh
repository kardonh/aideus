...
AIDEUS_HOME_DIR="${AIDEUS_HOME:-$HOME/.aideus}"
AIDEUS_CFG="${AIDEUS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Aideus config has twozero_td MCP entry ──
if [[ ! -f "$AIDEUS_CFG" ]]; then
    echo -e " ${FAIL} Aideus config not found at ${AIDEUS_CFG}"
    manual_steps+=("Create ${AIDEUS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AIDEUS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Aideus config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Aideus config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AIDEUS_CFG} manually"); }
    manual_steps+=("Restart Aideus session to pick up config change")
fi
