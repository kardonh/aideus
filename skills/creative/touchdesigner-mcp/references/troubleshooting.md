...
## 5. Aideus Configuration

### Config location

`$AIDEUS_HOME/config.yaml` (defaults to `~/.aideus/config.yaml` when `AIDEUS_HOME` is unset)
...
Restart the Aideus session for changes to take effect. The MCP connection is
