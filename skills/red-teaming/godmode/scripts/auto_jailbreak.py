...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("AIDEUS_HOME", os.path.expanduser("~/.aideus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AIDEUS_HOME", Path.home() / ".aideus")) / "skills" / "red-teaming" / "godmode"
...
AIDEUS_HOME = Path(os.getenv("AIDEUS_HOME", Path.home() / ".aideus"))
CONFIG_PATH = AIDEUS_HOME / "config.yaml"
PREFILL_PATH = AIDEUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Aideus models — already uncensored, just needs clean prompt
    "aideus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "aideus" in model_lower or "nous" in model_lower:
        return "aideus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Aideus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.aideus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Aideus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Aideus for changes to take effect.")
