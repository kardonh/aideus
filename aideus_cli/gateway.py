"""
Gateway subcommand for aideus CLI.

Handles: aideus gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_aideus_home is imported lazily at call sites to avoid ImportError
# when aideus_constants is cached from a pre-update version during `aideus update`.
from aideus_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from aideus_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "aideus-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "aideus_cli.main gateway",
        "aideus_cli.main --profile",
        "aideus_cli.main -p",
        "aideus_cli/main.py gateway",
        "aideus_cli/main.py --profile",
        "aideus_cli/main.py -p",
        "aideus gateway",
        "gateway/run.py",
    ]
    current_home = str(get_aideus_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"AIDEUS_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "AIDEUS_HOME=" in command and f"AIDEUS_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``aideus update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Aideus profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}aideus gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from aideus_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `aideus gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (AIDEUS_HOME-scoped).
...
from aideus_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "aideus-gateway"
SERVICE_DESCRIPTION = "Aideus Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current AIDEUS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (AIDEUS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from aideus_constants import get_default_aideus_root
    home = get_aideus_home().resolve()
    default = get_default_aideus_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary AIDEUS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(aideus_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when AIDEUS_HOME is a named profile.

    For ``~/.aideus/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        aideus_home: Optional explicit AIDEUS_HOME path. Defaults to the current
            ``get_aideus_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from aideus_constants import get_default_aideus_root
    home = Path(aideus_home or str(get_aideus_home())).resolve()
    default = get_default_aideus_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this AIDEUS_HOME.

    Default ``~/.aideus`` returns ``aideus-gateway`` (backward compatible).
    Profile ``~/.aideus/profiles/coder`` returns ``aideus-gateway-coder``.
    Any other AIDEUS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Aideus installs that predate the
# aideus-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (aideus-gateway-*.service) and unrelated third-party
# "aideus" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("aideus.service",)
...
def _find_legacy_aideus_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Aideus gateway units.

    Detects unit files installed by older Aideus versions that used a
    different service name (e.g. ``aideus.service`` before the rename to
    ``aideus-gateway.service``). When both a legacy unit and the current
    ``aideus-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``aideus-gateway-coder.service`` and unrelated third-party
      ``aideus-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``aideus.service`` running an unrelated
...
def has_legacy_aideus_units() -> bool:
    """Return True when any legacy Aideus gateway unit files exist."""
    return bool(_find_legacy_aideus_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Aideus gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_aideus_units()
    if not legacy:
        return
    print_warning("Legacy Aideus gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current aideus-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    aideus gateway migrate-legacy")


def remove_legacy_aideus_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Aideus gateway unit files.

    Iterates over whatever ``_find_legacy_aideus_units()`` returns — which is
    ...
    legacy = _find_legacy_aideus_units()
    if not legacy:
        print("No legacy Aideus gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Aideus gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: aideus gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo aideus gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    aideus gateway uninstall")
    print_info("    sudo aideus gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Aideus can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo aideus gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo aideus gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo aideus gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Aideus often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.aideus`` → ``ai.aideus.gateway.plist`` (backward compatible).
    Profile ``~/.aideus/profiles/coder`` → ``ai.aideus.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.aideus.gateway-{suffix}" if suffix else "ai.aideus.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.aideus/aideus-agent  -> /home/alice/.aideus/aideus-agent
      /opt/aideus                 -> /opt/aideus  (kept as-is)
...
def _aideus_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current AIDEUS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_aideus_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.aideus                    → /home/alice/.aideus
      /root/.aideus/profiles/coder     → /home/alice/.aideus/profiles/coder
      /opt/custom-aideus               → /opt/custom-aideus  (kept as-is)
    """
    current_aideus = get_aideus_home().resolve()
    current_default = (Path.home() / ".aideus").resolve()
    target_default = Path(target_home_dir) / ".aideus"

    # Default ~/.aideus → remap to target user's default
    if current_aideus == current_default:
        return str(target_default)

    # Profile or subdir of ~/.aideus → preserve the relative structure
    try:
        relative = current_aideus.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.aideus) — keep as-is
        return str(current_aideus)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        aideus_home = _aideus_home_for_target_user(home_dir)
        profile_arg = _profile_arg(aideus_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m aideus_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="AIDEUS_HOME={aideus_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    aideus_home = str(get_aideus_home().resolve())
    profile_arg = _profile_arg(aideus_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Aideus install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (aideus.service from pre-rename installs)
    # before installing the new aideus-gateway.service. If both remain, they
    ...
    if has_legacy_aideus_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_aideus_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}aideus gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}aideus gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `aideus gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}aideus gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}aideus gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}aideus gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}aideus gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.aideus.gateway-{suffix}" if suffix else "ai.aideus.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Aideus install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  aideus gateway status             # Check status")
    from aideus_constants import display_aideus_home as _dhh
...
def launchd_stop():
    ...
    # `aideus gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Aideus install")
    else:
        print("⚠ Service definition is stale relative to the current Aideus install")
        print("  Run: aideus gateway start")
    ...
    if deep:
        log_file = get_aideus_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Aideus will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @aideus:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. aideus) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Aideus delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Aideus agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Aideus will use (e.g., aideus@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Aideus connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: aideus pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Aideus will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``aideus setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.aideus/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_aideus_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'aideus pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: aideus pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'aideus gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from aideus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from aideus_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Aideus will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Aideus will store the returned account_id/token in ~/.aideus/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `aideus gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_aideus_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `aideus pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `aideus pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"AideusAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``aideus_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from aideus_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.aideus/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.aideus/.env: {', '.join(required)}")
    else:
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: aideus gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: aideus gateway install")
                else:
                    print_info("  You can install later: aideus gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo aideus gateway install --system")
                    print_info("  Or run in foreground:  aideus gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: aideus gateway run")
                print_info("  For persistence:   tmux new -s aideus 'aideus gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: aideus gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup aideus gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: aideus gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'aideus gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: aideus gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: aideus gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s aideus 'aideus gateway run'")
                print()
            ...
            print()
            print("  aideus gateway run                              # direct foreground")
            print("  tmux new -s aideus 'aideus gateway run'         # persistent via tmux")
            print("  nohup aideus gateway run > ~/.aideus/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: aideus gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: aideus gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: aideus gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: aideus gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: aideus gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  aideus gateway install")
                    print("  sudo aideus gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  aideus gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup aideus gateway run > ~/.aideus/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s aideus 'aideus gateway run'         # persistent via tmux")
                    print("  nohup aideus gateway run > ~/.aideus/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  aideus gateway install  # Install as user service")
                    print("  sudo aideus gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy Aideus gateway unit files from
        # pre-rename installs (e.g. aideus.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_aideus_units(interactive=not yes, dry_run=dry_run)