"""Slash command definitions and autocomplete for the Aideus CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Aideus config/state", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Aideus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.aideus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Aideus Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`aideus_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/aideus`` subcommand mapping, and
    ...
    """
    try:
        from aideus_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``aideus skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/aideus <verb>`` subcommand.
    ...
    avoidance. ``/aideus`` is always reserved as the first entry so the
    legacy ``/aideus <subcommand>`` form keeps working for anything that
    ...
    # Reserve /aideus as the catch-all top-level command.
    entries.append(("aideus", "Talk to Aideus or run a subcommand", "[subcommand] [args]"))
    seen.add("aideus")
...
def slack_app_manifest(request_url: str = "https://aideus-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /aideus handler.

    Maps both canonical names and aliases so /aideus bg do stuff works
    the same as /aideus background do stuff.

    Plugin-registered slash commands are included so ``/aideus <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from aideus_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
