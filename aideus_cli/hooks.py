"""aideus hooks — inspect and manage shell-script hooks.

Usage::

    aideus hooks list
    aideus hooks test <event> [--for-tool X] [--payload-file F]
    aideus hooks revoke <command>
    aideus hooks doctor

Consent records live under ``~/.aideus/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.aideus/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``aideus hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: aideus hooks {list|test|revoke|doctor}")
        print("Run 'aideus hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.aideus/config.yaml.")
        print("See `aideus hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `aideus hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `aideus hooks test` and `aideus hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Aideus wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `aideus hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `aideus hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `aideus hooks doctor`.")
