"""``aideus logs`` — view and filter Aideus log files.
...
Usage examples::
    ...
    aideus logs                    # last 50 lines of agent.log
    aideus logs -f                 # follow agent.log in real time
    aideus logs errors             # last 50 lines of errors.log
    aideus logs gateway -n 100    # last 100 lines of gateway.log
    aideus logs --level WARNING    # only WARNING+ lines
    aideus logs --session abc123   # filter by session ID substring
    aideus logs --component tools  # only tool-related lines
    aideus logs --since 1h         # lines from the last hour
    aideus logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_aideus_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Aideus runs — try 'aideus chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_aideus_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_aideus_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_aideus_home()}/logs/")
        return

    print(f"Log files in {display_aideus_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'aideus chat' to generate logs)")
