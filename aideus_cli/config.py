"""
Configuration management for Aideus Agent.

Config files are stored in ~/.aideus/ for easy access:
- ~/.aideus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.aideus/.env         - API keys and secrets

This module provides:
- aideus config          - Show current configuration
- aideus config edit     - Open config in editor
- aideus config set      - Set a specific value
- aideus config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `aideus plugins enable
    # observability/langfuse` or `aideus tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "AIDEUS_LANGFUSE_ENV",
    "AIDEUS_LANGFUSE_RELEASE",
    "AIDEUS_LANGFUSE_SAMPLE_RATE",
    "AIDEUS_LANGFUSE_MAX_CHARS",
    "AIDEUS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from aideus_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("AIDEUS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_aideus_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Aideus is running in package-manager-managed mode.

    Two signals: the AIDEUS_MANAGED env var (set by the systemd service),
    or a .managed marker file in AIDEUS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade aideus-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "aideus update"


def format_managed_message(action: str = "modify this Aideus installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("AIDEUS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Aideus installation is managed by NixOS "
            f"(AIDEUS_MANAGED={env_hint}).\n"
            "Edit services.aideus-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Aideus installation is managed by Homebrew "
            f"(AIDEUS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade aideus-agent"
        )

    return (
        f"Cannot {action}: this Aideus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Aideus."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from AIDEUS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, aideus_bin
    or None if container mode is not active, we're already inside the
    container, or AIDEUS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("AIDEUS_DEV") == "1":
        return None
    ...
    container_mode_file = get_aideus_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "aideus-agent")
    exec_user = info.get("exec_user", "aideus")
    aideus_bin = info.get("aideus_bin", "/data/current-package/bin/aideus")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "aideus_bin": aideus_bin,
    }
...
# Re-export from aideus_constants — canonical definition lives there.
from aideus_constants import get_aideus_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_aideus_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_aideus_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the aideus group can
    share state with the gateway service.

    The mode can be overridden via the AIDEUS_HOME_MODE environment variable
    (e.g. AIDEUS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse AIDEUS_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("AIDEUS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Aideus runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("AIDEUS_CONTAINER") or os.environ.get("AIDEUS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set AIDEUS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into AIDEUS_HOME if the user doesn't have one yet."""
...
def ensure_aideus_home():
    """Ensure ~/.aideus directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_aideus_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_aideus_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"AIDEUS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Aideus-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Aideus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Aideus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Aideus
        ...
        # bundled Aideus image, which drops to the `aideus` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Aideus sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.aideus/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, aideus sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: AIDEUS_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, Aideus truncates the
    ...
    "display": {
        ...
        # When true, `aideus --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `aideus -c` muscle memory.  Default off so existing
        # users aren't surprised.  AIDEUS_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/aideus`. Per-platform overrides go under
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.aideus/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.aideus/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${AIDEUS_SKILL_DIR} and ${AIDEUS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `aideus curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # to use Aideus' main auxiliary client resolution.
        "auxiliary": {
            "provider": None,
            "model": None,
        },
    ...
    # This section is only needed for aideus-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Aideus Agent*" header.
    ...
    # stored approval from ~/.aideus/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or AIDEUS_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via AIDEUS_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     aideus-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.aideus/logs/.
    ...
    # update model picker lists without shipping a aideus-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://aideus-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `aideus model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.aideus/state.db.
    ...
    "sessions": {
        ...
        # default of ``aideus sessions prune``.
        "retention_days": 90,
    ...
    # ``aideus update`` behaviour.
    "updates": {
        # Run a full ``aideus backup``-style zip of AIDEUS_HOME before every
        # ``aideus update``.  Backups land in ``<AIDEUS_HOME>/backups/`` and
        # can be restored with ``aideus import <path>``.  Off by default —
        # on large AIDEUS_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'aideus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Aideus auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @aideus:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. AIDEUS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #aideus)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: aideus-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'aideus-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Aideus API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Aideus API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Aideus API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # AIDEUS_TOOL_PROGRESS and AIDEUS_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "AIDEUS_TOOL_PROGRESS": {
    ...
    "AIDEUS_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.aideus.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Aideus versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``aideus_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'aideus setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Aideus won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'aideus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("AIDEUS_HOME", "~/.aideus")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("AIDEUS_TOOL_PROGRESS")
            old_mode = get_env_value("AIDEUS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from AIDEUS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from AIDEUS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("AIDEUS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from AIDEUS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$AIDEUS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_aideus_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `aideus plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: aideus config set <key> <value>")
    ...
    # metadata.aideus.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _aideus_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.aideus/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.aideus/config.yaml.
    ...
    (which change ``AIDEUS_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_aideus_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — aideus auth) — OpenAI Codex
#   nous         (OAuth — aideus auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.aideus/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.aideus/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Aideus env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.aideus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.aideus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.aideus/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.aideus/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Aideus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Aideus vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.aideus/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Aideus Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  aideus config edit     # Edit config file", Colors.DIM))
    print(color("  aideus config set <key> <value>", Colors.DIM))
    print(color("  aideus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: aideus config set <key> <value>")
            print()
            print("Examples:")
            print("  aideus config set model anthropic/claude-sonnet-4")
            print("  aideus config set terminal.backend docker")
            print("  aideus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'aideus config migrate' to add them")
        ...
        print("  aideus config           Show current configuration")
        print("  aideus config edit      Open config in editor")
        print("  aideus config set <key> <value>   Set a config value")
        print("  aideus config check     Check for missing/outdated config")
        print("  aideus config migrate   Update config with new options")
        print("  aideus config path      Show config file path")
        print("  aideus config env-path  Show .env file path")
        sys.exit(1)
