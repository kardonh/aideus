"""CLI subcommand: `aideus curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not args.synchronous:
        print("llm pass running in background — check `aideus curator status` later")
    return 0
...
# ---------------------------------------------------------------------------
# argparse wiring (called from aideus_cli.main)
# ---------------------------------------------------------------------------
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by aideus_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="aideus curator")
    register_cli(parser)
