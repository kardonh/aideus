"""
Single source of truth for provider identity in Aideus Agent.
...
2. **Aideus overlays** — transport type, auth patterns, aggregator flags,
...
# -- Aideus overlay ----------------------------------------------------------
# Aideus-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AideusOverlay:
    """Aideus-specific provider metadata layered on top of models.dev."""
...
AIDEUS_OVERLAYS: Dict[str, AideusOverlay] = {
    "openrouter": AideusOverlay(
    ...
    "openai-codex": AideusOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": AideusOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="AIDEUS_QWEN_BASE_URL",
    ),
    "google-gemini-cli": AideusOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": AideusOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": AideusOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AideusOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AideusOverlay(
    ...
    "kimi-for-coding": AideusOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": AideusOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": AideusOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": AideusOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": AideusOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": AideusOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": AideusOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": AideusOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": AideusOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": AideusOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AideusOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": AideusOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": AideusOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": AideusOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "aideus", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Aideus overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Aideus overlay
    ...
    overlay = AIDEUS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + aideus extra
    ...
    if overlay is not None:
        # Aideus-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in AIDEUS_OVERLAYS
    if provider == "bedrock":
