#!/usr/bin/env python3
"""
Aideus CLI - Main entry point.

Usage:
    aideus                     # Interactive chat (default)
    aideus chat                # Interactive chat
    aideus gateway             # Run gateway in foreground
    aideus gateway start       # Start gateway as service
    aideus gateway stop        # Stop gateway service
    aideus gateway status      # Show gateway status
    aideus gateway install     # Install gateway service
    aideus gateway uninstall   # Uninstall gateway service
    aideus setup               # Interactive setup wizard
    aideus logout              # Clear stored authentication
    aideus status              # Show status of all components
    aideus cron                # Manage cron jobs
    aideus cron list           # List cron jobs
    aideus cron status         # Check if cron scheduler is running
    aideus doctor              # Check configuration and dependencies
    aideus honcho setup                    # Configure Honcho AI memory integration
    aideus honcho status                   # Show Honcho config and connection status
    aideus honcho sessions                 # List directory → session name mappings
    aideus honcho map <name>               # Map current directory to a session name
    aideus honcho peer                     # Show peer names and dialectic settings
    aideus honcho peer --user NAME         # Set user peer name
    aideus honcho peer --ai NAME           # Set AI peer name
    aideus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    aideus honcho mode                     # Show current memory mode
    aideus honcho mode [hybrid|honcho|local]  # Set memory mode
    aideus honcho tokens                   # Show token budget settings
    aideus honcho tokens --context N       # Set session.context() token cap
    aideus honcho tokens --dialectic N     # Set dialectic result char cap
    aideus honcho identity                 # Show AI peer identity representation
    aideus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    aideus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Aideus + Honcho
    aideus version             Show version
    aideus update              Update to latest version
    aideus uninstall           Uninstall Aideus Agent
    aideus acp                 Run as an ACP server for editor integration
    aideus sessions browse     Interactive session picker with search

    aideus claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to AIDEUS_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (aideus tools, aideus setup, aideus model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'aideus {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any aideus module import.
#
# Many modules cache AIDEUS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("AIDEUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.aideus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set AIDEUS_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If AIDEUS_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("AIDEUS_HOME"):
        return

    # 2. If no flag, check active_profile in the aideus root
    if profile_name is None:
        try:
            from aideus_constants import get_default_aideus_root

            active_path = get_default_aideus_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set AIDEUS_HOME
    if profile_name is not None:
        try:
            from aideus_cli.profiles import resolve_profile_env

            aideus_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent aideus from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["AIDEUS_HOME"] = aideus_home
...
# Load .env from ~/.aideus/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from aideus_cli.config import get_aideus_home
from aideus_cli.env_loader import load_aideus_dotenv

load_aideus_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → AIDEUS_REDACT_SECRETS env
# var BEFORE aideus_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "AIDEUS_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_aideus_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["AIDEUS_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `aideus` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from aideus_logging import setup_logging as _setup_logging
...
try:
    from aideus_cli.config import load_config as _load_config_early
    from aideus_constants import apply_ipv4_preference as _apply_ipv4
...
from aideus_cli import __version__, __release_date__
from aideus_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from aideus_cli.config import get_env_path, get_aideus_home, load_config
    from aideus_cli.auth import get_auth_status

    # Determine whether Aideus itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from aideus_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_aideus_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from aideus_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_aideus_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Aideus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Aideus to use their tokens.
    if _has_aideus_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from aideus_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, aideus_bin
        cli_args: the original CLI arguments (everything after 'aideus')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    aideus_bin = container_info["aideus_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo aideus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo aideus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  aideus --tui --resume {target}")
    if title:
        print(f'  aideus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @aideus/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@aideus" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: AIDEUS_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("AIDEUS_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _aideus_ink_bundle_stale(tui_dir):
        return True
...
def _aideus_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "aideus-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``AIDEUS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("AIDEUS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    aideus_home = os.environ.get("AIDEUS_HOME") or str(Path.home() / ".aideus")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "AIDEUS_HOME": aideus_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(aideus_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (AIDEUS_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("AIDEUS_NODE")
    ...
    # pre-built dist + node_modules (nix / full AIDEUS_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("AIDEUS_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("AIDEUS_QUIET"):
    ...
    if tui_dev:
        if _aideus_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/aideus-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from aideus_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="aideus-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["AIDEUS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["AIDEUS_PYTHON_SRC_ROOT"] = os.environ.get(
        "AIDEUS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("AIDEUS_PYTHON", sys.executable)
    env.setdefault("AIDEUS_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["AIDEUS_MODEL"] = model
        env["AIDEUS_INFERENCE_MODEL"] = model
    if provider:
        env["AIDEUS_TUI_PROVIDER"] = provider
        env["AIDEUS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["AIDEUS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["AIDEUS_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("AIDEUS_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'aideus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Aideus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  aideus setup")
        print()

        from aideus_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'aideus setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from aideus_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["AIDEUS_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.aideus/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["AIDEUS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["AIDEUS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["AIDEUS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from aideus_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from aideus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Aideus?")
        print()
    ...
    session_dir = get_aideus_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: aideus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  aideus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Aideus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  aideus gateway")
        ...
        print()
        print("  Or install as a service: aideus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'aideus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from aideus_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``aideus model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from aideus_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from aideus_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from aideus_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("AIDEUS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'aideus model' for "
                "available providers, or run 'aideus doctor' to diagnose config "
                "issues."
            )
    ...
    from aideus_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from aideus_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.aideus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.aideus/.env if the active provider is not 'custom'.
    ...
    """
    from aideus_cli.config import get_env_value, save_env_value, load_config
...
#
# Aideus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `aideus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `aideus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from aideus_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Aideus only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``aideus model`` flow, then route aux tasks to them here.
    """
    from aideus_cli.config import load_config
    from aideus_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from aideus_cli.auth import _prompt_model_selection
    from aideus_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from aideus_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from aideus_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from aideus_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from aideus_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from aideus_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from aideus_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `aideus auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from aideus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from aideus_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from aideus_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from aideus_cli.auth import _save_model_choice, deactivate_provider
    from aideus_cli.config import get_env_value, load_config, save_config
    ...
    from aideus_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `aideus model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from aideus_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from aideus_cli.config import get_env_value, save_env_value, load_config, save_config
    from aideus_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Aideus will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from aideus_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from aideus_cli.config import save_env_value, load_config, save_config
    from aideus_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from aideus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from aideus_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Aideus turns to `copilot --acp`.")
    print("  Aideus currently starts its own ACP subprocess for each request.")
    print("  Aideus uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set AIDEUS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so aideus knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Aideus typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Aideus, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from aideus_cli.auth import AuthError
        from aideus_cli.models import fetch_lmstudio_models
        ...
        from aideus_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from aideus_constants import display_aideus_home as _dhh_fn

            print(
                f"    Aideus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from aideus_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'aideus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Aideus CLI with a provider."""
    from aideus_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from aideus_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from aideus_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from aideus_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from aideus_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from aideus_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``aideus slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: aideus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `aideus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from aideus_cli.slack_cli import slack_manifest_command
...
def cmd_hooks(args):
    ...
    from aideus_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from aideus_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from aideus_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from aideus_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from aideus_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Aideus home directory to a zip file."""
    if getattr(args, "quick", False):
        from aideus_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from aideus_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Aideus backup from a zip file."""
    from aideus_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Aideus Agent v{__version__} ({__release_date__})")
    ...
    try:
        from aideus_cli.banner import check_for_updates
        from aideus_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Aideus Agent."""
    _require_tty("uninstall")
    from aideus_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``aideus update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``aideus_cli/web_dist/`` (per vite.config.ts
    outDir: "../aideus_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "aideus_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``aideus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``aideus web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (aideus web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``aideus dashboard`` processes other than ourselves.

    ``aideus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``aideus update`` replaces files on disk, the running
    ...
    patterns = [
        "aideus dashboard",
        "aideus_cli.main dashboard",
        "aideus_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "aideus.*dashboard"`) keeps us consistent
            # with `aideus_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes() -> None:
    """Kill ``aideus dashboard`` processes left over from the previous version.

    Called at the end of ``aideus update``.  The dashboard has no service
    ...
    if killed:
        ...
        print("    aideus dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Aideus Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/Kardonh/aideus/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="aideus-update-")
        zip_path = os.path.join(tmp_dir, f"aideus-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to aideus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"aideus-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "aideus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Aideus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes aideus completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Aideus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Aideus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Aideus behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `aideus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Kardonh/aideus.git",
    "git@github.com:Kardonh/aideus.git",
    "https://github.com/Kardonh/aideus",
    "git@github.com:Kardonh/aideus",
}
OFFICIAL_REPO_URL = "https://github.com/Kardonh/aideus.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_aideus_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_aideus_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Aideus repository.")
        print("  This means you may miss updates from Kardonh/aideus.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Kardonh/aideus.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Kardonh/aideus.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``aideus update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from aideus_constants import get_default_aideus_root

    default_home = get_default_aideus_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "aideus-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``aideus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.aideus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``aideus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``aideus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.aideus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``aideus update --gateway``) the update is already
    ...
    try:
        ...
        # aideus_cli.config.get_aideus_home to simulate setup failure.
        from aideus_cli.config import get_aideus_home as _get_aideus_home

        logs_dir = _get_aideus_home() / "logs"
...
def _cmd_update_check():
    """Implement ``aideus update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``aideus update`` without requiring a reinstall.  The
    ...
    ``aideus`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/aideus.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v aideus'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/aideus, code at /usr/local/lib/aideus-agent).
    fhs_link = Path("/usr/local/bin/aideus")
    ...
    path_comment = (
        "# Aideus Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of AIDEUS_HOME before running the update.
    ...
    AIDEUS_HOME directories.  The ``--backup`` flag on ``aideus update``
    ...
    try:
        from aideus_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_aideus_home so the user sees ~/.aideus/...
    try:
        from aideus_constants import get_aideus_home, display_aideus_home
        home = get_aideus_home()
        try:
            display_path = f"{display_aideus_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  aideus import {out_path}")
...
def cmd_update(args):
    """Update Aideus Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from aideus_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Aideus Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_aideus_home added to aideus_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload aideus_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_aideus_home() added since the last release.
        try:
            import importlib
            import aideus_constants as _hc
        ...
        try:
            from aideus_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'aideus config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: aideus config migrate")
            else:
                print()
                print("Skipped. Run 'aideus config migrate' later to configure.")
        else:
        ...
        # When running as ``aideus update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_aideus_home() / ".update_exit_code"
            try:
        ...
        try:
            from aideus_cli.gateway import (
                is_macos,
            ...
            # Discover all aideus-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. aideus-gateway.service or aideus-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: aideus gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: aideus -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Aideus gateway unit files are still installed.
        # When both aideus.service (from a pre-rename install) and the
        # current aideus-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `aideus update` surfaces the issue until the user migrates.
        try:
            from aideus_cli.gateway import (
                has_legacy_aideus_units,
                _find_legacy_aideus_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_aideus_units():
                print()
                print("⚠ Legacy Aideus gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_aideus_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (aideus.service) fight the current")
                print("  aideus-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    aideus gateway migrate-legacy")
                print()
        ...
        print("  aideus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``aideus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from aideus_constants import display_aideus_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `aideus profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_aideus_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → aideus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.aideus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  aideus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     aideus -p {name} chat")
                else:
        ...
        from aideus_cli.profiles import profile_exists
        ...
        from aideus_cli.profiles import rename_profile
        ...
        from aideus_cli.profiles import export_profile
        ...
        from aideus_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    if "AIDEUS_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from aideus_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("AIDEUS_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from aideus_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Aideus log files."""
    from aideus_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for aideus CLI."""
    from aideus_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: aideus-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from aideus_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://aideus-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `aideus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy aideus.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Aideus gateway unit files "
            "(e.g. aideus.service) left over from older installs. Profile "
            "units (aideus-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Aideus Agent with an interactive wizard. "
        "Run a specific section: aideus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'aideus setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Aideus.",
    )
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $AIDEUS_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Aideus")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Aideus CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: aideus-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate Aideus with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set AIDEUS_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Aideus Agent components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.aideus/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.aideus/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Aideus Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Aideus setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Aideus Agent. Use 'aideus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    aideus debug share              Upload debug report and print URL
    aideus debug share --lines 500  Include more log lines
    aideus debug share --expire 30  Keep paste for 30 days
    aideus debug share --local      Print report locally (no upload)
    aideus debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'aideus debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Aideus home directory to a zip file",
        description="Create a zip archive of your entire Aideus configuration, "
        "skills, sessions, and data (excludes the aideus-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/aideus-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Aideus backup from a zip file",
        description="Extract a previously created Aideus backup into your "
        "Aideus home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Aideus Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from aideus_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.aideus/skills/.bundled_manifest) "
            "so future 'aideus update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from aideus_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from aideus_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/aideus-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `aideus plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from aideus_cli.plugins_cmd import plugins_command
    ...
    try:
        from aideus_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_aideus_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_aideus_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_aideus_home()}/memories/\n")
        else:
            from aideus_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'aideus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # aideus tools list [--platform cli]
    ...
    # aideus tools disable <name...> [--platform cli]
    ...
    # aideus tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from aideus_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from aideus_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Aideus as an MCP server",
        description=(
            "Manage MCP server connections and run Aideus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'aideus mcp add' to connect to a new server, or\n"
            "'aideus mcp serve' to expose Aideus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Aideus as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from aideus_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_aideus_home() / "sessions"
            ...
            # Launch aideus --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from aideus_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Aideus",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Aideus",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.aideus/ (by default a "
        "single restore-point archive is written to ~/.aideus/backups/ "
        "before apply; restorable with 'aideus import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from aideus_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Aideus Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Aideus Agent",
        description="Remove Aideus Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Aideus Agent as an ACP (Agent Client Protocol) server",
        description="Start Aideus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Aideus Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Aideus instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Aideus Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `aideus --tui` via PTY/WebSocket). "
            "Alternatively set AIDEUS_DASHBOARD_TUI=1."
        ),
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Aideus log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    aideus logs                    Show last 50 lines of agent.log
    aideus logs -f                 Follow agent.log in real time
    aideus logs errors             Show last 50 lines of errors.log
    aideus logs gateway -n 100     Show last 100 lines of gateway.log
    aideus logs --level WARNING    Only show WARNING and above
    aideus logs --session abc123   Filter by session ID
    aideus logs --component tools  Only show tool-related lines
    aideus logs --since 1h         Lines from the last hour
    aideus logs --since 30m -f     Follow, starting from 30 min ago
    aideus logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``aideus -c Pokemon Agent Dev`` → ``aideus -c 'Pokemon Agent Dev'``
    ...
    from aideus_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'aideus -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (aideus hooks list, cron
    ...
    if (
        ...
        try:
            from aideus_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
    ...
    if getattr(args, "oneshot", None):
        from aideus_cli.oneshot import run_oneshot
