"""
Aideus Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m aideus_cli.main web          # Start on http://127.0.0.1:9119
    python -m aideus_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["AIDEUS_WEB_DIST"]) if "AIDEUS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Aideus Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Aideus-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``aideus dashboard --tui``
# or AIDEUS_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.aideus/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_aideus_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "aideus-update": "aideus-update.log",
}
...
def _spawn_aideus_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``aideus <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``aideus_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "aideus_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "AIDEUS_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``aideus gateway restart`` in the background."""
    try:
        proc = _spawn_aideus_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/aideus/update")
async def update_aideus():
    """Kick off ``aideus update`` in the background."""
    try:
        proc = _spawn_aideus_action(["update"], "aideus-update")
    except Exception as exc:
        _log.exception("Failed to spawn aideus update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Aideus supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in aideus_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.aideus/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``aideus auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Aideus resolves Anthropic creds in this order at runtime:
    1. ``~/.aideus/.anthropic_oauth.json`` — Aideus-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_aideus_oauth_credentials,
            read_claude_code_credentials,
            _AIDEUS_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_aideus_oauth_credentials = None  # type: ignore
        _AIDEUS_OAUTH_FILE = None  # type: ignore

    aideus_creds = None
    if read_aideus_oauth_credentials:
        try:
            aideus_creds = read_aideus_oauth_credentials()
        except Exception:
            aideus_creds = None
    if aideus_creds and aideus_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "aideus_pkce",
            "source_label": f"Aideus PKCE ({_AIDEUS_OAUTH_FILE})",
            "token_preview": _truncate_token(aideus_creds.get("accessToken")),
            "expires_at": aideus_creds.get("expiresAt"),
            "has_refresh_token": bool(aideus_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Aideus even
    when they also have a separate Aideus-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "aideus auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "aideus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "aideus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "aideus auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "aideus auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from aideus_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("aideus_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Aideus-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _AIDEUS_OAUTH_FILE
            if _AIDEUS_OAUTH_FILE.exists():
                _AIDEUS_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from aideus_cli.auth import clear_provider_auth
...
#          → persists to ~/.aideus/.anthropic_oauth.json AND credential pool
...
# Guarded so aideus web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Aideus file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``aideus auth add anthropic``.
    """
    from agent.anthropic_adapter import _AIDEUS_OAUTH_FILE
    payload = {
    ...
    _AIDEUS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _AIDEUS_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "aideus-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from aideus_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("AIDEUS_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from aideus_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from aideus_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from aideus_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_aideus_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from aideus_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from aideus_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from aideus_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``aideus --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``aideus --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``AIDEUS_TUI_RESUME`` env var —
    matching what ``aideus_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``AIDEUS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from aideus_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume:
        env["AIDEUS_TUI_RESUME"] = resume

    if sidecar_url:
        env["AIDEUS_TUI_SIDECAR_URL"] = sidecar_url
...
# AIDEUS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__AIDEUS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__AIDEUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};</script>"
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Aideus Teal",  "description": "Classic dark teal — the canonical Aideus look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.aideus/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.aideus/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_aideus_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.aideus/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as aideus_cli.plugins):
    1. User plugins:    ~/.aideus/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.aideus/plugins/  (only if AIDEUS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from aideus_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_aideus_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("AIDEUS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".aideus" / "plugins", "project"))
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"aideus_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Aideus Web UI → http://{host}:{port}")
