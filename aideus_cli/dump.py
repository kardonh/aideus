"""
Dump command for aideus CLI.

Outputs a compact, plain-text summary of the user's Aideus setup
...
from aideus_cli.config import get_aideus_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``aideus dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from aideus_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(aideus_home: Path) -> int:
...
def _cron_summary(aideus_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = aideus_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"aideus_home:      {display_aideus_home()}")
    ...
    toolsets = config.get("toolsets", ["aideus-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(aideus_home)}")
    lines.append(f"  skills:             {_count_skills(aideus_home)}")
