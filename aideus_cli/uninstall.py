"""
Aideus Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.aideus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Aideus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing aideus-agent or aideus PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Aideus Agent" comment and following line
                if '# Aideus Agent' in line or '# aideus-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('aideus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing aideus
                if 'aideus' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the aideus wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "aideus",
        Path("/usr/local/bin/aideus"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains aideus_cli reference)
                content = wrapper.read_text()
                if 'aideus_cli' in content or 'aideus-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``aideus gateway run`` processes
    ...
    try:
        from aideus_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from aideus_cli.gateway import get_launchd_plist_path
...
def _is_default_aideus_home(aideus_home: Path) -> bool:
    """Return True when ``aideus_home`` points at the default (non-profile) root."""
    try:
        from aideus_constants import get_default_aideus_root
        return aideus_home.resolve() == get_default_aideus_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its AIDEUS_HOME directory.

    We shell out to ``aideus -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current AIDEUS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m aideus_cli.main` so we don't depend on a `aideus`
    #    wrapper that may be half-removed mid-uninstall.
    aideus_invocation = [_sys.executable, "-m", "aideus_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                aideus_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's AIDEUS_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.aideus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.aideus/ for future reinstall
    """
    project_root = get_project_root()
    aideus_home = get_aideus_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie AIDEUS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_aideus_home(aideus_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Aideus Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {aideus_home / 'config.yaml'}")
    print(f"  Secrets: {aideus_home / '.env'}")
    print(f"  Data:    {aideus_home / 'cron/'}, {aideus_home / 'sessions/'}, {aideus_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their AIDEUS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Aideus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Aideus code but keep your configuration and data.")
    ...
    log_info("Removing aideus command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.aideus/, just remove the aideus-agent subdir
            if aideus_home in project_root.parents or project_root.parent == aideus_home:
    ...
    # 5. Optionally remove ~/.aideus/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile AIDEUS_HOME dirs live under
        ...
        try:
            if aideus_home.exists():
                shutil.rmtree(aideus_home)
                log_success(f"Removed {aideus_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {aideus_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {aideus_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/Kardonh/aideus/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using Aideus Agent! ⚕")
    print()
