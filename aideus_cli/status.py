"""
Status command for aideus CLI.

Shows the status of all Aideus Agent components.
"""
...
from aideus_cli.auth import AuthError, resolve_provider
from aideus_cli.colors import Colors, color
from aideus_cli.config import get_env_path, get_env_value, get_aideus_home, load_config
from aideus_cli.models import provider_label
from aideus_cli.nous_subscription import get_nous_subscription_features
from aideus_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``aideus config``'s
...
def show_status(args):
    """Show status of all Aideus Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Aideus Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: aideus auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: aideus model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: aideus auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: aideus model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from aideus_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'aideus-agent[vercel]')"
    ...
    try:
        from aideus_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   aideus gateway")
    ...
    jobs_file = get_aideus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_aideus_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'aideus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'aideus setup' to configure", Colors.DIM))
    print()
