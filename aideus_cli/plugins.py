"""
Aideus Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with aideus-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.aideus/plugins/<name>/``
3. **Project plugins** – ``./.aideus/plugins/<name>/`` (opt-in via
   ``AIDEUS_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``aideus_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``AIDEUS_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("AIDEUS_BUNDLED_PLUGINS")
    if env_override:
...
ENTRY_POINTS_GROUP = "aideus_agent.plugins"

_NS_PARENT = "aideus_plugins"
...
class PluginManifest:
    ...
    #              in ~/.aideus/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``aideus plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``aideus honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``aideus <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.aideus/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to aideus_cli/. Two layouts are
        ...
        # 2. User plugins (~/.aideus/plugins/)
        user_dir = get_aideus_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.aideus/plugins/)
        if _env_enabled("AIDEUS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".aideus" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with aideus and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Aideus ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `aideus plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``aideus_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``aideus_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``aideus tools`` TUI so plugin-provided toolsets appear
