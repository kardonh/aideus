...
- ``aideus_cli.providers``        -- canonical provider identity + overlays
- ``aideus_cli.model_normalize``  -- per-provider name formatting
...
_AIDEUS_MODEL_WARNING = (
    ...
    "for use with Aideus Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"aideus" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``aideus-brain:qwen3-14b-ctx16k`` that just
# happen to carry "aideus" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   aideus-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def _check_aideus_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _AIDEUS_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from aideus_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'aideus model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'aideus doctor' — config issues detected:"
    ...
    # Mirrors the same logic in aideus_cli.runtime_provider.resolve_runtime_provider;
    ...
    aideus_warn = _check_aideus_model_warning(new_model)
    if aideus_warn:
        warnings.append(aideus_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from aideus_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from aideus_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by aideus provider ID
    ...
    # --- 1. Check Aideus-mapped providers ---
    for aideus_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(aideus_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (AIDEUS_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from aideus_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and aideus_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Aideus release.
        model_ids = curated.get(aideus_id, [])
        if aideus_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(aideus_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Aideus-only providers (nous, openai-codex, copilot, opencode-go) ---
    from aideus_cli.providers import AIDEUS_OVERLAYS
    from aideus_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Aideus provider ID.
    # AIDEUS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Aideus IDs ("copilot").
    _mdev_to_aideus = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in AIDEUS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Aideus slug — e.g. "github-copilot" → "copilot"
        aideus_slug = _mdev_to_aideus.get(pid, pid)
        if aideus_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or aideus_slug in providers_store
                    or pid in pool_store or aideus_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(aideus_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", aideus_slug, exc)
        ...
        # configured.
        if not has_creds and aideus_slug == "anthropic":
            try:
                ...
                if (aideus_creds and aideus_creds.get("accessToken")) or \
        ...
        if aideus_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(aideus_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(aideus_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(aideus_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Aideus slug, fall back to overlay key
            model_ids = curated.get(aideus_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if aideus_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(aideus_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": aideus_slug,
            "name": get_label(aideus_slug),
            "is_current": aideus_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "aideus",
        ...
        seen_slugs.add(aideus_slug.lower())
        _record_builtin_endpoint(aideus_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or AIDEUS_OVERLAYS (keeps /model in sync
    # with `aideus model`).
    try:
        from aideus_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Aideus's canonical write key (matches
            ...
            # Aideus writes ``models:`` as a dict keyed by model id
            # (see aideus_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Aideus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Aideus's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, aideus_cli/config.py) already consume that dict.
