"""
Profile management for multiple isolated Aideus instances.

Each profile is a fully independent AIDEUS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.aideus/profiles/<name>/`` by default.

The "default" profile is ``~/.aideus`` itself — backward compatible,
zero migration needed.

Usage::

    aideus profile create coder          # fresh profile + bundled skills
    aideus profile create coder --clone  # also copy config, .env, SOUL.md
    aideus profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    aideus -p coder chat                 # or via flag
    aideus profile use coder             # set as sticky default
    aideus profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See aideus_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.aideus`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.aideus) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "aideus-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "aideus", "default", "test", "tmp", "root", "sudo",
})

# Aideus subcommands that cannot be used as profile names/aliases
_AIDEUS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the aideus root, NOT to the current AIDEUS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where AIDEUS_HOME points outside
    ``~/.aideus``, profiles live under ``AIDEUS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_aideus_home() / "profiles"


def _get_default_aideus_home() -> Path:
    """Return the default (pre-profile) AIDEUS_HOME path.

    In standard deployments this is ``~/.aideus``.
    In Docker/custom deployments where AIDEUS_HOME is outside ``~/.aideus``
    (e.g. ``/opt/data``), returns AIDEUS_HOME directly.
    """
    from aideus_constants import get_default_aideus_root
    return get_default_aideus_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_aideus_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.aideus
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its AIDEUS_HOME directory."""
    if name == "default":
        return _get_default_aideus_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, aideus subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _AIDEUS_SUBCOMMANDS:
        return f"'{name}' conflicts with a aideus subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_aideus_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.aideus)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.aideus/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches AIDEUS_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AIDEUS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.aideus).\n"
            "To remove everything, use: aideus uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set AIDEUS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("AIDEUS_HOME")
    try:
        os.environ["AIDEUS_HOME"] = str(profile_dir)
        from aideus_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["AIDEUS_HOME"] = old_home
        elif "AIDEUS_HOME" in os.environ:
            del os.environ["AIDEUS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.aideus/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: aideus profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from AIDEUS_HOME.

    Returns ``"default"`` if AIDEUS_HOME is not set or points to ``~/.aideus``.
    Returns the profile name if AIDEUS_HOME points into ``~/.aideus/profiles/<name>``.
    Returns ``"custom"`` if AIDEUS_HOME is set to an unrecognized path.
    """
    from aideus_constants import get_aideus_home
    aideus_home = get_aideus_home()
    resolved = aideus_home.resolve()

    default_resolved = _get_default_aideus_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.aideus itself — its parent is ~/ and its
        # directory name is ".aideus", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: aideus profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.aideus itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.aideus). "
            "Specify a different name: aideus profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="aideus_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"aideus.{old_name}"
    new_host = f"aideus.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_aideus_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for aideus profile names."""
    return '''# Aideus Agent profile completion
# Add to ~/.bashrc: eval "$(aideus completion bash)"

_aideus_profiles() {
    local profiles_dir="$HOME/.aideus/profiles"
...
_aideus_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_aideus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _aideus_completion aideus
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for aideus profile names."""
    return '''#compdef aideus
# Aideus Agent profile completion
# Add to ~/.zshrc: eval "$(aideus completion zsh)"

_aideus() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.aideus/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.aideus/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a AIDEUS_HOME path string.

    Called early in the CLI entry point, before any aideus modules
    are imported, to set the AIDEUS_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: aideus profile create {profile_name}"
        )
