...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or aideus -c.",
    ...
    "Ctrl+Z suspends Aideus to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "aideus -c resumes your most recent CLI session. aideus -c \"project name\" resumes by title.",
    "aideus -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "aideus -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "aideus chat -t web,terminal enables only specific toolsets for a focused session.",
    "aideus chat -s github-pr-workflow preloads a skill at launch.",
    "aideus chat -q \"query\" runs a single non-interactive query and exits.",
    "aideus chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "aideus chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "aideus --yolo bypasses all dangerous command approval prompts for the entire session.",
    "aideus chat --source telegram tags the session for filtering in aideus sessions list.",
    "aideus -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "aideus doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "aideus dump outputs a compact setup summary — great for bug reports.",
    "aideus config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "aideus config edit opens config.yaml in your default editor.",
    "aideus config check scans for missing or stale configuration options.",
    "aideus sessions browse opens an interactive session picker with search.",
    "aideus sessions stats shows session counts by platform and database size.",
    "aideus sessions prune --older-than 30 cleans up old sessions.",
    "aideus skills search react --source skills-sh searches the skills.sh public directory.",
    "aideus skills check scans installed hub skills for upstream updates.",
    "aideus skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "aideus skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "aideus mcp add github --command npx adds MCP servers from the command line.",
    "aideus mcp serve runs Aideus itself as an MCP server for other agents.",
    "aideus auth add lets you add multiple API keys for credential pool rotation.",
    "aideus completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "aideus logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "aideus backup creates a zip backup of your entire Aideus home directory.",
    "aideus profile create coder creates an isolated profile that becomes its own command.",
    "aideus profile create work --clone copies your current config and keys to a new profile.",
    "aideus update syncs new bundled skills to ALL profiles automatically.",
    "aideus gateway install sets up Aideus as a system service (systemd/launchd).",
    "aideus memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "aideus webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as aideus -w).",
    ...
    "AIDEUS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Aideus tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'aideus profile create coder' creates the 'coder' command.",
    "aideus profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Aideus prints a resume command with session ID and stats.",
    "aideus sessions export backup.jsonl exports all sessions for backup or analysis.",
    "aideus -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "aideus skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .aideus/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: aideus cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Aideus runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "aideus gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Aideus via a local macOS server.",
    ...
    "Context files (.aideus.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.aideus/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Aideus loads project context from .aideus.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — aideus tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.aideus/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "aideus chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "aideus plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "aideus acp runs Aideus as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "AIDEUS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "aideus login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "aideus status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.aideus/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Aideus your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.aideus/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Aideus auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.aideus/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at AIDEUS_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "AIDEUS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in AIDEUS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.aideus/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'aideus profile create ls' would be rejected.",
    "aideus profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "aideus claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "AIDEUS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via aideus tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.aideus/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.aideus/skills/.hub/quarantine/ holds skills pending security review.",
]
