# OpenAI-Compatible API Server for Aideus Agent
...
SSE streaming. By exposing this endpoint, aideus-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  aideus-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.aideus/.env`
2. Run `aideus gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with aideus-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns aideus-agent as a model) |
...
Authorization: Bearer aideus-api-key-here
...
But aideus-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "aideus-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured aideus-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of aideus-agent's core prompt
...
Once implemented, aideus-agent works as a drop-in backend for:
