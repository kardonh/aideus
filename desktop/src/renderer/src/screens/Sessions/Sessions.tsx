...
function Sessions({
  ...
  const loadSessions = useCallback(async (): Promise<void> => {
    setLoading(true);
    const cached = await window.aideusAPI.listCachedSessions(50);
    if (cached.length > 0) {
      setSessions(cached);
      setLoading(false);
    }
    const synced = await window.aideusAPI.syncSessionCache();
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      const results = await window.aideusAPI.searchSessions(searchQuery);
