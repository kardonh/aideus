...
function Soul({ profile }: SoulProps): React.JSX.Element {
  ...
  const loadSoul = useCallback(async (): Promise<void> => {
    ...
    setLoading(true);
    const text = await window.aideusAPI.readSoul(profile);
    setContent(text);
    setLoading(false);
    setTimeout(() => {
      loaded.current = true;
  ...
  const saveSoul = useCallback(
    async (text: string) => {
      ...
      await window.aideusAPI.writeSoul(text, profile);
      setSaved(true);
  ...
  async function handleReset(): Promise<void> {
    const newContent = await window.aideusAPI.resetSoul(profile);
    loaded.current = false;
