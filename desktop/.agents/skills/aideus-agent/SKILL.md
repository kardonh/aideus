...
# Aideus Agent - Complete Project Guide (A-Z)

> **Purpose of this document:** A single, comprehensive reference that explains everything about the Aideus Agent project — its architecture, source code, features, release history, and design patterns — so that any AI or developer can fully understand the system.
...
5. [Core Architecture](#5-core-architecture)
   ...
   - 5.5 [Session Database (aideus_state.py)](#55-session-database-aideus_statepy)
   - 5.6 [Constants & Home Directory (aideus_constants.py)](#56-constants--home-directory-aideus_constantspy)
6. [CLI System](#6-cli-system)
   - 6.1 [Interactive CLI (cli.py)](#61-interactive-cli-clipy)
   - 6.2 [CLI Entry Point (aideus_cli/main.py)](#62-cli-entry-point-aideus_climainpy)
   - 6.3 [Configuration System (aideus_cli/config.py)](#63-configuration-system-aideus_cliconfigpy)
   - 6.4 [Slash Command Registry (aideus_cli/commands.py)](#64-slash-command-registry-aideus_clicommandspy)
   - 6.5 [Setup Wizard (aideus_cli/setup.py)](#65-setup-wizard-aideus_clisetupy)
   - 6.6 [Model Catalog (aideus_cli/models.py)](#66-model-catalog-aideus_climodelspy)
   - 6.7 [Skin/Theme Engine (aideus_cli/skin_engine.py)](#67-skintheme-engine-aideus_cliskin_enginepy)
...
**Aideus Agent** is a self-improving AI agent built by [Nous Research](https://nousresearch.com). It is an open-source (MIT licensed), Python-based project that provides:
...
- **MCP integration** — both client (connect to any MCP server) and server (expose Aideus to MCP clients)
...
**Repository:** `github.com/Kardonh/aideus`
...
aideus              # start chatting

# Key commands
aideus model        # Choose LLM provider and model
aideus tools        # Configure which tools are enabled
aideus config set   # Set individual config values
aideus gateway      # Start the messaging gateway
aideus setup        # Run the full setup wizard
aideus update       # Update to latest version
aideus doctor       # Diagnose any issues
```
...
├── toolsets.py               # Toolset definitions, _AIDEUS_CORE_TOOLS list
├── toolset_distributions.py  # Toolset sampling distributions for RL
├── cli.py                    # AideusCLI class — interactive CLI orchestrator
├── aideus_state.py           # SessionDB — SQLite session store (FTS5 search)
├── aideus_constants.py       # Shared constants, get_aideus_home()
├── aideus_time.py            # Timezone handling
...
│
├── aideus_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `aideus` subcommands
│   ├── config.py             # DEFAULT_CONFIG, OPTIONAL_ENV_VARS, migration
│   ├── commands.py           # Slash command definitions + SlashCommandCompleter
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   ├── setup.py              # Interactive setup wizard
│   ├── skin_engine.py        # Skin/theme engine
│   ├── skills_config.py      # `aideus skills` — skill management
│   ├── tools_config.py       # `aideus tools` — tool management
...
│   ├── server.py             # AideusACPAgent class
...
│   ├── aideus_base_env.py    # Abstract base RL environment
│   ├── agent_loop.py         # AideusAgentLoop — rollout execution
...
- `aideus-cli` — All core tools for CLI platform
- `aideus-telegram`, `aideus-discord`, etc. — Platform-specific tool sets
- `aideus-gateway` — Union of all platform tools
...
### 5.5 Session Database (aideus_state.py)
...
### 5.6 Constants & Home Directory (aideus_constants.py)

Import-safe constants module with no circular dependencies.

```python
get_aideus_home() → Path          # AIDEUS_HOME env var or ~/.aideus
display_aideus_home() → str       # User-friendly display: "~/.aideus"
get_optional_skills_dir() → Path  # AIDEUS_OPTIONAL_SKILLS env var
...
The `AideusCLI` class provides the interactive terminal interface.
...
# Loads from ~/.aideus/config.yaml (or ./cli-config.yaml fallback)
...
### 6.2 CLI Entry Point (aideus_cli/main.py)

All `aideus` subcommands are dispatched from here:

```
aideus                    # Default: interactive chat
aideus chat               # Explicit interactive mode
aideus gateway start|stop|status|install|uninstall
aideus setup              # Setup wizard
aideus model              # Select model/provider
aideus tools              # Configure tools
aideus skills             # Manage skills
aideus config set|get     # Direct config manipulation
aideus cron list|delete   # Cron job management
aideus doctor             # Diagnose issues
aideus sessions browse    # Session picker
aideus profile create|list|switch|delete|export|import
aideus mcp serve|add|remove  # MCP management
aideus acp                # Start ACP server
aideus update|uninstall|version
```

**Profile System:**

- `_apply_profile_override()` runs BEFORE any imports to set `AIDEUS_HOME`
- Pre-parses `--profile/-p` from argv
- Allows fully isolated agent instances with separate config, memory, sessions, skills

---

### 6.3 Configuration System (aideus_cli/config.py)
...
- `~/.aideus/config.yaml` — User settings (authoritative)
- `~/.aideus/.env` — API keys and secrets
- Config version migration system (currently v5)

---

### 6.4 Slash Command Registry (aideus_cli/commands.py)
...
### 6.5 Setup Wizard (aideus_cli/setup.py)
...
### 6.6 Model Catalog (aideus_cli/models.py)
...
### 6.7 Skin/Theme Engine (aideus_cli/skin_engine.py)
...
**User Skins:** Drop `~/.aideus/skins/<name>.yaml` and activate with `/skin <name>`
...
| Skills Index            | `~/.aideus/skills/.aideus-skills.json`               |
| Platform Hints          | OS, Python version, shell, available tools           |
| Context Files           | `.aideus.md`, `AGENTS.md`, `.cursorrules`, `SOUL.md` |
| Model/Provider Info     | Current model and provider identity                  |

**Context File Discovery:**

1. Check `cwd/.aideus.md` or `AIDEUS.md`
...
- Skills loaded from `~/.aideus/skills/` and external directories
- Injected as **user message** (not system prompt) to preserve prompt caching
- `/plan` command generates implementation plans stored in `.aideus/plans/`
...
```
~/.aideus/sessions/{session_key}.json
...
| **Slack**          | Multi-workspace OAuth, thread handling, app_mention, `/aideus` subcommands                               |
...
**Job Storage:** `~/.aideus/cron/jobs.json`

**Execution Flow:**

1. `tick()` called every 60s from gateway background thread
2. Fetch due jobs past `next_run_at`
3. Spawn `aideus` CLI subprocess with job prompt + skills
4. Capture output → save to `~/.aideus/cron/output/{job_id}/{timestamp}.md`
...
- Auto-discovered from `~/.aideus/skills/` + external dirs
- Skills index built at startup (`.aideus-skills.json`)
- Loaded as user messages to preserve prompt caching
- Per-platform enable/disable via `aideus skills`
...
Drop Python files into `~/.aideus/plugins/` to extend Aideus.
...
- Enable/disable via `aideus plugins enable/disable <name>`
...
Aideus has a pluggable memory provider interface:
...
**Entry:** `aideus acp` → `acp_adapter/server.py`

**AideusACPAgent Class:**
...
- `X-Aideus-Session-Id` header for persistent sessions
...
Expose Aideus conversations to MCP-compatible clients.

**Entry:** `aideus mcp serve`
...
**Base Class:** `AideusAgentBaseEnv` (extends `atroposlib.BaseEnv`)

**Configuration:**

```python
AideusAgentEnvConfig:
...
- `aideus_swe_env.py` — Software engineering tasks

**Supporting:**

- `AideusAgentLoop` — Orchestrates step-by-step rollouts
...
Run multiple fully isolated Aideus instances from the same installation.

**Commands:**

```bash
aideus profile create <name>
aideus profile list
aideus profile switch <name>
aideus profile delete <name>
aideus profile export <name>
aideus profile import <file>
aideus -p <name>             # Launch with specific profile
```

**Each profile gets:**

- Own `AIDEUS_HOME` directory (`~/.aideus/profiles/<name>/`)
...
- `_apply_profile_override()` sets `AIDEUS_HOME` env var before any imports
- All 119+ references to `get_aideus_home()` automatically scope to active profile
- Profile operations are HOME-anchored (`~/.aideus/profiles/`) for cross-profile visibility
...
- Git worktree isolation (`aideus -w`)
...
- First-class plugin architecture (`~/.aideus/plugins/`)
...
- MCP Server Mode (`aideus mcp serve`)
...
```
aideus_constants.py  (no deps — imported by everything)
       ↑
...
model_tools.py  (imports tools/registry + triggers tool discovery)
       ↑
run_agent.py (AIAgent), cli.py (AideusCLI), gateway/run.py (GatewayRunner)
       ↑
aideus_cli/main.py  (entry point — dispatches to all subsystems)
```
...
| **Profile Isolation**          | AIDEUS_HOME env var set before imports; all state functions route through `get_aideus_home()`      |
...
| Variable              | Purpose                                                   |
| --------------------- | --------------------------------------------------------- |
| `AIDEUS_HOME`         | Override home directory (profiles set this automatically) |
...
| File                          | Purpose                          |
| ----------------------------- | -------------------------------- |
| `~/.aideus/config.yaml`       | Main configuration (YAML)        |
| `~/.aideus/.env`              | API keys and secrets             |
| `~/.aideus/MEMORY.md`         | Persistent agent memory          |
| `~/.aideus/USER.md`           | User profile                     |
| `~/.aideus/SOUL.md`           | Agent persona/identity           |
| `~/.aideus/sessions.db`       | SQLite session database          |
| `~/.aideus/cron/jobs.json`    | Cron job definitions             |
| `.aideus.md` (in project dir) | Per-project context file         |
...
1. **DO NOT hardcode `~/.aideus` paths** — Use `get_aideus_home()` from `aideus_constants`. Hardcoding breaks profiles.
...
6. **Tests must not write to `~/.aideus/`** — `_isolate_aideus_home` autouse fixture redirects to temp dir.
...
9. **Config has three loaders:** `load_cli_config()` (CLI), `load_config()` (aideus tools/setup), direct YAML (gateway). They have different merge behaviors.

10. **Profile operations are HOME-anchored** — `_get_profiles_root()` returns `Path.home() / ".aideus" / "profiles"`, NOT `get_aideus_home() / "profiles"`. This is intentional for cross-profile visibility.

---

_This document covers Aideus v0.7.0 as of April 2026. For the latest information, refer to the [official documentation](https://aideus.nousresearch.com/docs/) and the [GitHub repository](https://github.com/Kardonh/aideus)._
