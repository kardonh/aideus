  ...
  async copySelectionNoClear(): Promise<string> {
    ...
    if (text) {
      try {
        ...
        if (process.env.AIDEUS_TUI_DEBUG_CLIPBOARD) {
          console.error(
            '[clipboard] no path reached the clipboard (headless + no tmux?) — set AIDEUS_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      } catch (err) {
        if (process.env.AIDEUS_TUI_DEBUG_CLIPBOARD) {
