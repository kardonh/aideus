...
React + Ink terminal UI for Aideus. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `AIDEUS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.aideus/.aideus_history` or under `AIDEUS_HOME`.
...
```text
ui-tui/
  packages/aideus-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      aideus-ink.d.ts    type declarations for @aideus/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AideusCLI subprocess for slash commands
```
