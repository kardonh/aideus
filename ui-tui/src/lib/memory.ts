...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.AIDEUS_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.aideus', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `aideus-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
