import type { RunExternalProcess } from '@aideus/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`aideus ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching aideus: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`aideus ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
