# Contributing to Aideus Agent

Thank you for contributing to Aideus Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Aideus to work everywhere.
...
Bundled skills (in `skills/`) ship with every Aideus install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `aideus skills browse` (labeled "official") and install it with `aideus skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `aideus skills install`.
...
```bash
git clone --recurse-submodules https://github.com/Kardonh/aideus.git
cd aideus-agent
...
```bash
mkdir -p ~/.aideus/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.aideus/config.yaml
touch ~/.aideus/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.aideus/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/aideus" ~/.local/bin/aideus
...
├── cli.py                    # AideusCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (aideus-cli, aideus-telegram, etc.)
├── aideus_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── aideus_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.aideus/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (aideus-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.aideus/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.aideus/`)

| Path | Purpose |
|------|---------|
| `~/.aideus/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.aideus/.env` | API keys and secrets |
| `~/.aideus/auth.json` | OAuth credentials (Nous Portal) |
| `~/.aideus/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.aideus/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.aideus/state.db` | SQLite session database |
| `~/.aideus/sessions/` | JSON session logs |
| `~/.aideus/cron/` | Scheduled job data |
| `~/.aideus/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`aideus_state.py`) with full-text search and unique session titles. JSON logs go to `~/.aideus/sessions/`.
...
Four fields are supported under `metadata.aideus`:
...
The user may skip setup and keep loading the skill. Aideus only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `aideus setup` or update `~/.aideus/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Aideus tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `aideus --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Aideus uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.aideus/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `aideus_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `aideus_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Aideus runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
Aideus has terminal access. Security matters.
...
2. **Test manually**: Run `aideus` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Kardonh/aideus/issues)
- Include: OS, Python version, Aideus version (`aideus version`), full error traceback
