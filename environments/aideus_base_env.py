"""
AideusAgentBaseEnv -- Abstract Base Environment for Aideus-Agent + Atropos

Provides the Atropos integration plumbing that all aideus-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via AideusAgentLoop
...
# Ensure the aideus-agent repo root is on sys.path so that imports like
...
# Load API keys from aideus-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, AideusAgentLoop
...
# Import aideus-agent toolset infrastructure
...
class AideusAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for aideus-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of aideus toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: aideus, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/aideus-results/ "
...
class AideusAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for aideus-agent Atropos integration.
    ...
    name: Optional[str] = "aideus-agent"
    env_config_cls = AideusAgentEnvConfig

    def __init__(
        self,
        config: AideusAgentEnvConfig,
        ...
        # Set terminal environment variables so aideus tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY aideus-agent tool (terminal, file, web,
