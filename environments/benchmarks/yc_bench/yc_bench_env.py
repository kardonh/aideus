...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs AideusAgentLoop with terminal tool only
...
Requires: pip install aideus-agent[yc-bench]
"""
...
class YCBenchEvalConfig(AideusAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends AideusAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(AideusAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The AideusAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs AideusAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our AideusAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the AideusAgentLoop
            # ----------------------------------------------------------
