"""
AideusAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the aideus-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to aideus-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by AideusAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by AideusAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class AideusAgentLoop:
    """
    Runs aideus-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # aideus-agent's standalone parsers. This handles the case where
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via aideus-agent's dispatch
