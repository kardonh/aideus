# Aideus-Agent Atropos Environments

This directory contains the integration layer between **aideus-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  AideusAgentBaseEnv    │  aideus_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     AideusSweEnv    TerminalBench2EvalEnv
...
**AideusAgentBaseEnv** (`aideus_base_env.py`) extends BaseEnv with aideus-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves aideus-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `AideusAgentBaseEnv` and implement:
...
`AideusAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as aideus-agent's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** aideus-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any aideus-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some aideus-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `aideus` -- Aideus/ChatML `<tool_call>` XML format
...
├── aideus_base_env.py            # Abstract base (AideusAgentBaseEnv)
...
│
├── aideus_swe_env/               # SWE-bench style training environment
│   └── aideus_swe_env.py
│
...
2. Create your env file inheriting from `AideusAgentBaseEnv`
...
2. Inherit from `AideusAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which aideus toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `aideus` |
