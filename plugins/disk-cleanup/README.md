...
Auto-tracks and cleans up ephemeral files created during Aideus Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$AIDEUS_HOME` and `/tmp/aideus-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `AIDEUS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `AIDEUS_HOME` or `/tmp/aideus-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$AIDEUS_HOME/disk-cleanup/` is itself excluded
- `$AIDEUS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
