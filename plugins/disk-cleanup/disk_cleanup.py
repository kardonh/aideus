"""disk_cleanup — ephemeral file cleanup for Aideus Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under AIDEUS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly AIDEUS_HOME and /tmp/aideus-*
Never touches: ~/.aideus/logs/ or any system directory.
"""
...
try:
    ...
    def get_aideus_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("AIDEUS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".aideus").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$AIDEUS_HOME/logs/``."""
    return get_aideus_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$AIDEUS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under AIDEUS_HOME or ``/tmp/aideus-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    aideus_home = get_aideus_home()
    try:
        path.resolve().relative_to(aideus_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/aideus-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("aideus-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside AIDEUS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under AIDEUS_HOME (but leave AIDEUS_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "aideus-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(aideus_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == aideus_home:
                continue
            try:
                rel_parts = dirpath.relative_to(aideus_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    aideus_home = get_aideus_home()
    try:
        rel = path.resolve().relative_to(aideus_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "aideus-agent",
        }:
            return None
        ...
        # Path isn't under AIDEUS_HOME (e.g. /tmp/aideus-*) — fall through.
        pass
