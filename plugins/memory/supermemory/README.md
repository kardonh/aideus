...
Config file: `$AIDEUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `aideus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `aideus-{identity}` → `aideus-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Aideus profile:

```json
{
  "container_tag": "aideus-{identity}"
}
```

For a profile named `coder`, this resolves to `aideus-coder`. The default profile resolves to `aideus-default`. Without `{identity}`, all profiles share the same container.
