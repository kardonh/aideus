...
def _load_supermemory_config(aideus_home: str) -> dict:
    config = _default_config()
    config_path = Path(aideus_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, aideus_home: str) -> None:
    config_path = Path(aideus_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `aideus memory setup`.
        # All other options are documented for $AIDEUS_HOME/supermemory.json
    ...
    def save_config(self, values, aideus_home):
        ...
        _save_supermemory_config(sanitized, aideus_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from aideus_constants import get_aideus_home
        self._aideus_home = kwargs.get("aideus_home") or str(get_aideus_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._aideus_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "aideus", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "aideus_memory", "target": target, "type": "explicit_memory"},
