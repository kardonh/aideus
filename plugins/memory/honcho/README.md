...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/aideus>
...
```bash
aideus honcho setup    # full interactive wizard (cloud or local)
aideus memory setup    # generic picker, also works
```

Or manually:
```bash
aideus config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.aideus/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$AIDEUS_HOME/honcho.json` | Profile-local (isolated Aideus instances) |
| 2 | `~/.aideus/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Aideus profile: `aideus` (default) or `aideus.<profile>`.
...
| 4 | `per-session` strategy | Aideus session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`aideus-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`aideus`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-aideus-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening aideus in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Aideus session ID (timestamp + hex). Every `aideus` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Aideus profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`aideus`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Aideus profile: `aideus` (default) or `aideus.<profile>` (e.g. `aideus -p coder` → host key `aideus.coder`).
...
| `AIDEUS_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `aideus honcho setup` | Full interactive setup wizard |
| `aideus honcho status` | Show resolved config for active profile |
| `aideus honcho enable` / `disable` | Toggle Honcho for active profile |
| `aideus honcho mode <mode>` | Change recall or observation mode |
| `aideus honcho peer --user <name>` | Update user peer name |
| `aideus honcho peer --ai <name>` | Update AI peer name |
| `aideus honcho tokens --context <N>` | Set context token budget |
| `aideus honcho tokens --dialectic <N>` | Set dialectic max chars |
| `aideus honcho map <name>` | Map current directory to a session name |
| `aideus honcho sync` | Create host blocks for all Aideus profiles |
