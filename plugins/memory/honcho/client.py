...
Resolution order for config file:
  1. $AIDEUS_HOME/honcho.json  (instance-local, enables isolated Aideus instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Aideus profile.

    Resolution order:
      1. AIDEUS_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``aideus.<profile>``
      3. Fallback: ``"aideus"`` (default profile)
    """
    explicit = os.environ.get("AIDEUS_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $AIDEUS_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.aideus/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_aideus_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".aideus" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Aideus system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.aideus
    ...
    def from_global_config(
        ...
        Resolution: $AIDEUS_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Aideus profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.aideus block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Aideus session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Aideus session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Aideus session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'aideus honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            aideus_cfg = load_config()
            honcho_cfg = aideus_cfg.get("honcho", {})
