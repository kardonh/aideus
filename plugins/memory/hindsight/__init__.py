...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: aideus)
...
Or via $AIDEUS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Aideus can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $AIDEUS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_aideus_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "aideus": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "aideus"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Aideus config."""
    profile = config.get("profile", "aideus")
    return str(profile or "aideus")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Aideus profile name (from agent_identity)
      {workspace} — Aideus workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``aideus-{user}`` with no user becomes ``aideus``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Aideus Agent and the User"
    ...
    def save_config(self, values, aideus_home):
        """Write config to $AIDEUS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(aideus_home) / "hindsight"
    ...
    def post_setup(self, aideus_home: str, config: dict) -> None:
        ...
        from aideus_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "aideus")
        ...
        self.save_config(provider_config, aideus_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(aideus_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(aideus_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "aideus"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: aideus-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Aideus Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "aideus"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "aideus"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "aideus", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "aideus")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Aideus Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See aideus-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Aideus created/used that client on the shared async loop,
