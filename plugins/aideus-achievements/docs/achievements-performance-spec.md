# Aideus Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: aideus-achievements plugin
...
### `GET /api/plugins/aideus-achievements/achievements`
...
### `POST /api/plugins/aideus-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/aideus-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/aideus-achievements/overview`
...
- Persist plugin data under:
  - `~/.aideus/plugins/aideus-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.aideus/plugins/aideus-achievements/scan_checkpoint.json`
...
- `~/.aideus/plugins/aideus-achievements/`
