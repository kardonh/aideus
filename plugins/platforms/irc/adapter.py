"""
IRC Platform Adapter for Aideus Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Aideus agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "aideus-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Aideus Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Aideus Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: aideus_, aideus_1, aideus_2...
...
def check_requirements() -> bool:
    ...
    # aideus setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `aideus gateway setup` flow for the IRC platform.

    Lazy-imports ``aideus_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Aideus to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #aideus — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.aideus/.env")
    print_info("Restart the gateway for changes to take effect: aideus gateway restart")
