"""langfuse — Aideus plugin for Langfuse observability.

Traces Aideus conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Aideus plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``aideus plugins enable
observability/langfuse`` or ``aideus tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``aideus tools`` or ~/.aideus/.env):
  AIDEUS_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  AIDEUS_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  AIDEUS_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  AIDEUS_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  AIDEUS_LANGFUSE_RELEASE     - release/version tag
  AIDEUS_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  AIDEUS_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  AIDEUS_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("AIDEUS_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Aideus plugin system —
    ...
    public_key = _env("AIDEUS_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("AIDEUS_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    if not (public_key and secret_key):
        _LANGFUSE_CLIENT = _INIT_FAILED
        return None

    base_url = _env("AIDEUS_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("AIDEUS_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("AIDEUS_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("AIDEUS_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid AIDEUS_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("AIDEUS_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Aideus turn",
                tags=["aideus", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Aideus branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Aideus also has a turn-scoped
    ...
    # messages is a list only for legacy Aideus branches that fired
    # pre_llm_call with API messages directly. Current Aideus fires
...
def register(ctx) -> None:
    ...
    # Aideus versions.  pre_api_request / post_api_request fire per API
