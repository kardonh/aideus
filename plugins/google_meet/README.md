...
Let the aideus agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where aideus runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `aideus meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `aideus meet` CLI |
...
| `cli.py` | `aideus meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$AIDEUS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `aideus meet node {run,list,approve,remove,status,ping}` |
...
aideus meet install                                      # pip + Chromium
aideus meet setup                                        # preflight
aideus meet auth                                         # optional
aideus meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
aideus meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.aideus/.env
aideus meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
aideus meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
aideus meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.aideus/.env
aideus meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, aideus will **not** switch your system audio input automatically — the
...
aideus plugins enable google_meet
aideus meet node run --display-name my-mac --host 0.0.0.0 --port 18789
