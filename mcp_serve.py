"""
Aideus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Aideus-specific extra)

Usage:
    aideus mcp serve
    aideus mcp serve --verbose
...
logger = logging.getLogger("aideus.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using AIDEUS_HOME."""
    try:
        from aideus_constants import get_aideus_home
        return get_aideus_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("AIDEUS_HOME", Path.home() / ".aideus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from aideus_constants import get_aideus_home
        directory_file = get_aideus_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("AIDEUS_HOME", Path.home() / ".aideus")
...
class EventBridge:
    ...
    This is the Aideus equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from aideus_constants import get_aideus_home
            db_file = get_aideus_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("AIDEUS_HOME", Path.home() / ".aideus")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Aideus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "aideus",
        instructions=(
            "Aideus Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Aideus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
