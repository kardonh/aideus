...
def _isolate(tmp_path, monkeypatch):
    """Isolate AIDEUS_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal AideusCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AideusCLI.__new__(AideusCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["aideus-core"]
        obj.compact = False
