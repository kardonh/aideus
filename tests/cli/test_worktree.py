...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("aideus/aideus-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned aideus/* and pr-* branches."""

    def test_prunes_orphaned_aideus_branch(self, git_repo):
        """aideus/aideus-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "aideus/aideus-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "aideus/aideus-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "aideus/aideus-deadbeef" in result.stdout
        ...
        assert "aideus/aideus-deadbeef" in orphaned
        ...
        assert "aideus/aideus-deadbeef" not in result.stdout
