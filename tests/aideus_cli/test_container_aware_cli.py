...
def container_env(tmp_path, monkeypatch):
    """Set up a fake AIDEUS_HOME with .container-mode file."""
    aideus_home = tmp_path / ".aideus"
    aideus_home.mkdir()
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
    monkeypatch.delenv("AIDEUS_DEV", raising=False)

    container_mode = aideus_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=aideus-agent\n"
        "exec_user=aideus\n"
        "aideus_bin=/data/current-package/bin/aideus\n"
    )
    return aideus_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("aideus_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "aideus-agent"
    assert info["exec_user"] == "aideus"
    assert info["aideus_bin"] == "/data/current-package/bin/aideus"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("aideus_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_aideus_dev(container_env, monkeypatch):
    """Returns None when AIDEUS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("AIDEUS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_aideus_dev_zero(container_env, monkeypatch):
    """AIDEUS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("AIDEUS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        aideus_home = Path(tmpdir) / ".aideus"
        aideus_home.mkdir()
        (aideus_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("aideus_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_aideus_home": lambda: aideus_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AIDEUS_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=aideus-custom\n"
        "exec_user=myuser\n"
        "aideus_bin=/opt/aideus/bin/aideus\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "aideus-custom"
    assert info["exec_user"] == "myuser"
    assert info["aideus_bin"] == "/opt/aideus/bin/aideus"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from aideus_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "aideus"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "aideus-agent" in cmd
    assert "/data/current-package/bin/aideus" in cmd
    assert "chat" in cmd
