...
Ensures ``aideus update`` correctly detects running gateways managed by
...
from aideus_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """aideus_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units aideus-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="aideus-gateway.service loaded active running Aideus Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>AIDEUS_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: aideus gateway run'."""
        ...
        assert "Restart manually: aideus gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: aideus gateway run" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted aideus-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="aideus-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "aideus_cli.gateway._graceful_restart_via_sigusr1",
...
class TestServicePidExclusion:
    ...
    where ``aideus update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.aideus.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="aideus-gateway.service loaded inactive dead Aideus Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.aideus/aideus-agent/venv/bin/python -m aideus_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.aideus/aideus-agent/venv/bin/python -m aideus_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda aideus_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``aideus update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point AIDEUS_HOME at a temp dir so the marker file lands there
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
        import aideus_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_aideus_home", lambda: aideus_home)
        # Also patch the module-level ref used by cmd_update
        import aideus_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_aideus_home", lambda: aideus_home)
        ...
        exit_code_path = aideus_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy aideus.service warning printed by `aideus update`.

    Users who installed Aideus before the service rename often have a
    dormant ``aideus.service`` that starts flap-fighting the current
    ``aideus-gateway.service`` after PR #5646. Every ``aideus update``
    should remind them to run ``aideus gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "aideus.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Aideus gateway unit(s) detected" in captured
        assert "aideus.service" in captured
        assert "aideus gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Aideus gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (aideus-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``aideus.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "aideus-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "aideus-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Aideus gateway" not in captured
        assert "aideus-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Aideus gateway" in captured
