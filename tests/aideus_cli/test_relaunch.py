"""Tests for aideus_cli.relaunch — unified self-relaunch utility."""
...
class TestResolveAideusBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        ...
        assert relaunch_mod.resolve_aideus_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "aideus"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'aideus' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_aideus_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/aideus" if name == "aideus" else None
        )
        assert relaunch_mod.resolve_aideus_bin() == "/usr/bin/aideus"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_aideus_bin() is None
...
class TestBuildRelaunchArgv:
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/aideus", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/aideus", ["/usr/bin/aideus", "--resume", "abc"])]