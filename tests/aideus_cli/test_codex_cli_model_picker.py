...
Covers:
 - Normal path (tokens already in Aideus auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Aideus
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `aideus auth openai-codex`. The old
...
@pytest.fixture()
def aideus_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Aideus auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(aideus_auth_only_env):
    """openai-codex appears when tokens are already in Aideus auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Aideus
    auth store."""
    aideus_home = tmp_path / ".aideus"
    aideus_home.mkdir()
    ...
    (aideus_home / "auth.json").write_text(
