"""Unit tests for aideus_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf aideus-ok"])
        try:
            output = _read_until(bridge, b"aideus-ok")
            assert b"aideus-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$AIDEUS_PTY_TEST\""],
            env={**os.environ, "AIDEUS_PTY_TEST": "pty-env-works"},
        )
        try:
