"""Tests for the `aideus memory reset` CLI command.
...
- Profile-scoped reset (uses AIDEUS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake AIDEUS_HOME with memory files."""
    aideus_home = tmp_path / ".aideus"
    memories = aideus_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAideus repo is at ~/.aideus/aideus-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `aideus memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        (aideus_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's AIDEUS_HOME."""
        ...
        monkeypatch.setenv("AIDEUS_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

        # The memories dir won't exist; get_aideus_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
