...
import aideus_cli.doctor as doctor
import aideus_cli.gateway as gateway_cli
from aideus_cli import doctor as doctor_mod
from aideus_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "AIDEUS_HOME", aideus_home)
    monkeypatch.delenv("AIDEUS_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("AIDEUS_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_aideus_home(self, tmp_path, provider=""):
        """Create a minimal AIDEUS_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_aideus_home(tmp_path, provider)
...
def test_run_doctor_accepts_aideus_provider_ids_that_catalog_aliases(
