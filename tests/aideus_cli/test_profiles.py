"""Comprehensive tests for aideus_cli.profiles module.
...
from aideus_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_aideus_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and AIDEUS_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.aideus/profiles)
    * AIDEUS_HOME  -> tmp_path/.aideus  (so get_aideus_home() agrees)
    * Creates the bare-minimum ~/.aideus directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".aideus"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("AIDEUS_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_aideus_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".aideus"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".aideus" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.aideus must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".aideus"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("aideus_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".aideus" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_aideus_home_returns_default(self, profile_env):
        # AIDEUS_HOME points to tmp_path/.aideus which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".aideus" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom AIDEUS_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("AIDEUS_HOME", str(custom))
        # With Docker-aware roots, a custom AIDEUS_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("aideus")
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "aideus.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["aideus.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["aideus.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "aideus.ssi_health": {"workspace": "aideus", "enabled": True}
            }
        }))
        ...
        assert "aideus.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["aideus.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["aideus.heimdall"]["workspace"] == "aideus"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "aideus.ssi_health": {"aiPeer": "ssi_health"},
                "aideus.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["aideus.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["aideus.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("aideus-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".aideus_history",
        ...
        excluded_prefixes = [
            "default/aideus-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.aideus_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_aideus_profiles_function(self):
        script = generate_bash_completion()
        assert "_aideus_profiles" in script

    def test_zsh_completion_has_aideus_function(self):
        script = generate_zsh_completion()
        assert "_aideus" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultAideusHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_aideus_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".aideus" / "profiles"

    def test_default_aideus_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_aideus_home()
        assert home == tmp_path / ".aideus"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (AIDEUS_HOME outside ~/.aideus), profiles go under AIDEUS_HOME."""
    ...
    def test_default_aideus_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_aideus_home() returns AIDEUS_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AIDEUS_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (AIDEUS_HOME under ~/.aideus), profiles root is still ~/.aideus/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under AIDEUS_HOME."""
        from aideus_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AIDEUS_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under AIDEUS_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AIDEUS_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".aideus" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from aideus_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".aideus"
