...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "aideus" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_aideus_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:aideus_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:aideus_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "aideus_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "aideus-cli",
    ...
    # `aideus auth add nous` must also populate providers.nous so the
    ...
    # raises "Aideus is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`aideus auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "aideus_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from aideus_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from aideus_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`aideus logout --provider openai-codex` must still clear model.provider.
    ...
    (aideus_home / "config.yaml").write_text(
        "model:\n"
    ...
    config_text = (aideus_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `aideus logout` should target configured Codex if auth has no active provider."""
    aideus_home = tmp_path / "aideus"
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (aideus_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((aideus_home / "auth.json").read_text())
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from aideus_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "aideus_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("aideus_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("aideus_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("aideus_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "aideus_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (aideus_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from aideus_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((aideus_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from aideus_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from aideus_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `aideus auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("AIDEUS_HOME", str(tmp_path / "aideus"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `aideus auth remove`)
    (aideus_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((aideus_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aideus_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((aideus_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`aideus auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.aideus/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.aideus/.env (not the shell), the
    ...
    (aideus_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (aideus_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `aideus auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from aideus_cli.auth import is_source_suppressed
    from aideus_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `aideus auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Aideus reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import aideus_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_aideus_pkce_suppression(tmp_path, monkeypatch):
    """anthropic aideus_pkce must not re-seed from ~/.aideus/.anthropic_oauth.json when suppressed."""
    aideus_home = tmp_path / "aideus"
    aideus_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

    import yaml
    (aideus_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (aideus_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["aideus_pkce"]},
    }))

    # Stub the readers so only aideus_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_aideus_oauth_credentials", lambda: {
    ...
    # aideus_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "aideus_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (aideus_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `aideus auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `aideus auth add openai-codex`)
