"""Tests for Codex auth — tokens stored in Aideus auth store (~/.aideus/auth.json)."""
...
def _setup_aideus_auth(aideus_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Aideus auth store."""
    ...
    auth_file = aideus_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_aideus_auth(aideus_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    aideus_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (aideus_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    aideus_home = tmp_path / "aideus"
    _setup_aideus_auth(aideus_home, access_token="")
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_aideus_auth(aideus_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
    ...
    monkeypatch.setattr("aideus_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    aideus_home = tmp_path / "aideus"
    _setup_aideus_auth(aideus_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Aideus auth store, not ~/.codex/."""
    aideus_home = tmp_path / "aideus"
    codex_home = tmp_path / "codex-cli"
    aideus_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "aideus-at", "refresh_token": "aideus-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Aideus store
    assert not (codex_home / "auth.json").exists()

    # Aideus auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "aideus-at"


def test_resolve_returns_aideus_auth_store_source(tmp_path, monkeypatch):
    aideus_home = tmp_path / "aideus"
    _setup_aideus_auth(aideus_home)
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "aideus-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("aideus_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "aideus_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "aideus_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("aideus_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("aideus_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
