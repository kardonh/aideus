"""Tests for the top-level `./aideus` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./aideus` should use `aideus_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "aideus"
    called = []

    fake_main_module = types.ModuleType("aideus_cli.main")

    def fake_main():
        called.append("aideus_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "aideus_cli.main", fake_main_module)
