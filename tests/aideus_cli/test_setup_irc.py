"""Tests for IRC gateway configuration via `aideus setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Aideus install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Aideus install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#aideus")
            monkeypatch.setenv("IRC_NICKNAME", "aideus-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `aideus setup gateway` experience with IRC present."""
