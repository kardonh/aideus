"""Tests for aideus_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("aideus_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from aideus_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("aideus_cli.config.load_config")
    ...
    @patch("aideus_cli.config.load_config")
    @patch.dict("os.environ", {"AIDEUS_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("AIDEUS_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """AIDEUS_SESSION_PLATFORM should be used when AIDEUS_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_aideus_platform_takes_precedence(self, tmp_path, monkeypatch):
        """AIDEUS_PLATFORM should win over AIDEUS_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path))
        monkeypatch.setenv("AIDEUS_PLATFORM", "telegram")
        monkeypatch.setenv("AIDEUS_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path))
        monkeypatch.setenv("AIDEUS_PLATFORM", "telegram")
        monkeypatch.setenv("AIDEUS_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from aideus_cli.skills_config import _get_categories
        skills = [
