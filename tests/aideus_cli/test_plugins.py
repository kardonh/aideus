"""Tests for the Aideus plugin system (aideus_cli.plugins)."""
...
from aideus_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<aideus_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<aideus_home>/plugins/``; we derive
    ``<aideus_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <AIDEUS_HOME>/config.yaml.
        # Config is always read from AIDEUS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        aideus_home_str = os.environ.get("AIDEUS_HOME")
        if aideus_home_str:
            aideus_home = Path(aideus_home_str)
        else:
            aideus_home = base.parent
        aideus_home.mkdir(parents=True, exist_ok=True)
        cfg_path = aideus_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.aideus/plugins/ are discovered."""
        plugins_dir = tmp_path / "aideus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path / "aideus_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AIDEUS_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".aideus" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        aideus_home = tmp_path / "aideus_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under aideus_plugins.<name>."""
        plugins_dir = tmp_path / "aideus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path / "aideus_test"))

        # Clean up any prior namespace module
        sys.modules.pop("aideus_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "aideus_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="aideus_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "aideus_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import aideus_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("aideus_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / "aideus_test"
        plugins_dir = aideus_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "aideus_test" / "plugins"
        # Set AIDEUS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("aideus_cli.plugins.PluginContext.dispatch_tool.__module__", "aideus_cli.plugins"):
