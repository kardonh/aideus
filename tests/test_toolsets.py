...
class TestToolsetConsistency:
    ...
    def test_aideus_platforms_share_core_tools(self):
        """All aideus-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on aideus-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["aideus-cli", "aideus-telegram", "aideus-discord", "aideus-whatsapp", "aideus-slack", "aideus-signal", "aideus-homeassistant"]
