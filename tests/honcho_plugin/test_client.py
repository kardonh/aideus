...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "aideus"
        assert config.workspace_id == "aideus"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.aideus/honcho.json
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"aideus": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"aideus": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"aideus": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="aideus-agent"
        ):
            result = config.resolve_session_name("/home/user/aideus-agent/subdir")
        assert result == "aideus-agent"
...
class TestResolveConfigPath:
    def test_prefers_aideus_home_when_exists(self, tmp_path):
        aideus_home = tmp_path / "aideus"
        aideus_home.mkdir()
        local_cfg = aideus_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        aideus_home = tmp_path / "aideus"
        aideus_home.mkdir()
        # No honcho.json in AIDEUS_HOME — also isolate ~/.aideus so
    ...
    def test_falls_back_to_global_without_aideus_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("AIDEUS_HOME", None)
            result = resolve_config_path()
        assert result == fake_home / ".honcho" / "config.json"
...
class TestResolveActiveHost:
    def test_default_returns_aideus(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("AIDEUS_HONCHO_HOST", None)
            os.environ.pop("AIDEUS_HOME", None)
            assert resolve_active_host() == "aideus"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"AIDEUS_HONCHO_HOST": "aideus.coder"}):
            assert resolve_active_host() == "aideus.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AIDEUS_HONCHO_HOST", None)
            with patch("aideus_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "aideus.coder"

    def test_default_profile_returns_aideus(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AIDEUS_HONCHO_HOST", None)
            with patch("aideus_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "aideus"

    def test_custom_profile_returns_aideus(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AIDEUS_HONCHO_HOST", None)
            with patch("aideus_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "aideus"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AIDEUS_HONCHO_HOST", None)
            # Temporarily remove aideus_cli.profiles to simulate import failure
            saved = sys.modules.get("aideus_cli.profiles")
            sys.modules["aideus_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "aideus"
            finally:
                if saved is not None:
                    sys.modules["aideus_cli.profiles"] = saved
                else:
                    sys.modules.pop("aideus_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="aideus.coder")
        assert config.host == "aideus.coder"
        assert config.workspace_id == "aideus"  # shared workspace
        assert config.ai_peer == "aideus.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="aideus")
        assert config.host == "aideus"
        assert config.workspace_id == "aideus"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "aideus": {"aiPeer": "aideus", "peerName": "alice"},
                "aideus.coder": {
                    "aiPeer": "aideus.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="aideus.coder", config_path=config_file,
        )
        assert config.host == "aideus.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "aideus.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="aideus.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "aideus.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"aideus": {"enabled": True, "aiPeer": "aideus"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"aideus": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_aideus_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="aideus",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("aideus_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("aideus_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_aideus_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="aideus",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("aideus_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
