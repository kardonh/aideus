...
class TestAideusConstantsFallback:
    """Tests for _aideus_home.py fallback when aideus_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_aideus_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _aideus_home.py with aideus_constants blocked."""
        monkeypatch.setitem(sys.modules, "aideus_constants", None)
        spec = importlib.util.spec_from_file_location("_aideus_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_aideus_home_env_var(self, monkeypatch, tmp_path):
        """When aideus_constants is missing, AIDEUS_HOME comes from env var."""
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path / "custom-aideus"))
        module = self._load_helper(monkeypatch)
        assert module.get_aideus_home() == tmp_path / "custom-aideus"

    def test_fallback_defaults_to_dot_aideus(self, monkeypatch):
        """When aideus_constants is missing and AIDEUS_HOME unset, default to ~/.aideus."""
        monkeypatch.delenv("AIDEUS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_aideus_home() == Path.home() / ".aideus"

    def test_fallback_ignores_empty_aideus_home(self, monkeypatch):
        """Empty/whitespace AIDEUS_HOME is treated as unset."""
        monkeypatch.setenv("AIDEUS_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_aideus_home() == Path.home() / ".aideus"

    def test_fallback_display_aideus_home_shortens_path(self, monkeypatch):
        """Fallback display_aideus_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("AIDEUS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_aideus_home() == "~/.aideus"

    def test_fallback_display_aideus_home_profile_path(self, monkeypatch):
        """Fallback display_aideus_home() handles profile paths under ~/."""
        monkeypatch.setenv("AIDEUS_HOME", str(Path.home() / ".aideus/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_aideus_home() == "~/.aideus/profiles/coder"

    def test_fallback_display_aideus_home_custom_path(self, monkeypatch):
        """Fallback display_aideus_home() returns full path for non-home locations."""
        monkeypatch.setenv("AIDEUS_HOME", "/opt/aideus-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_aideus_home() == "/opt/aideus-custom"

    def test_delegates_to_aideus_constants_when_available(self):
        """When aideus_constants IS importable, _aideus_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_aideus_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import aideus_constants
        assert module.get_aideus_home is aideus_constants.get_aideus_home
        assert module.display_aideus_home is aideus_constants.display_aideus_home
