...
def server():
    with patch.dict("sys.modules", {
        "aideus_constants": MagicMock(get_aideus_home=MagicMock(return_value="/tmp/aideus_test")),
        "aideus_cli.env_loader": MagicMock(),
        "aideus_cli.banner": MagicMock(),
        "aideus_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `AIDEUS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("AIDEUS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("AIDEUS_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/aideus-agent-dev": {"name": "aideus-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "aideus-agent-dev", "session_id": sid},
        })
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "aideus-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "aideus-agent-dev"
