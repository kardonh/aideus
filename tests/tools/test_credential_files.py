...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        aideus_home.mkdir()
        (aideus_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"AIDEUS_HOME": str(aideus_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(aideus_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.aideus/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        (aideus_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        (aideus_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        (aideus_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.aideus/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        aideus_home = tmp_path / ".aideus"
        (aideus_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"AIDEUS_HOME": str(aideus_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.aideus")

        assert mounts[0]["container_path"] == "/home/user/.aideus/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.aideus/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.aideus/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape AIDEUS_HOME."""
        monkeypatch.setenv("AIDEUS_HOME", str(tmp_path / ".aideus"))
        (tmp_path / ".aideus").mkdir()

        # Create a sensitive file one level above aideus_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside aideus_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside AIDEUS_HOME must still be registered."""
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
        (aideus_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_aideus_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of AIDEUS_HOME must be allowed."""
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        subdir = aideus_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside AIDEUS_HOME pointing outside must be rejected."""
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

        # Create a sensitive file outside aideus_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside aideus_home pointing outside
        symlink = aideus_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes AIDEUS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, aideus_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape AIDEUS_HOME."""
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
        ...
        sensitive.write_text("{}")
        self._write_config(aideus_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(aideus_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside AIDEUS_HOME via config must still mount."""
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

        (aideus_home / "oauth.json").write_text("{}")
        self._write_config(aideus_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        (aideus_home / "cache" / "documents").mkdir(parents=True)
        (aideus_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.aideus/cache/documents" in paths
        assert "/root/.aideus/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.aideus/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir()
        # Use legacy dir name — get_aideus_dir prefers old if it exists
        (aideus_home / "document_cache").mkdir()
        (aideus_home / "image_cache").mkdir()
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(aideus_home / "document_cache") in host_paths
        assert str(aideus_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.aideus/cache/documents" in container_paths
        assert "/root/.aideus/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        doc_dir = aideus_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        ss_dir = aideus_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.aideus/cache/screenshots/session_abc/screen1.png"
