...
class TestWriteDenyExactPaths:
    ...
    def test_aideus_env(self):
        # ``.env`` under the active AIDEUS_HOME (profile-aware, not just
        # ``~/.aideus``) must be write-denied. The hermetic test conftest
        # points AIDEUS_HOME at a tempdir — resolve via get_aideus_home()
        # to match the denylist.
        from aideus_constants import get_aideus_home
        path = str(get_aideus_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_aideus_config_not_env(self):
        path = os.path.join(str(Path.home()), ".aideus", "config.yaml")
