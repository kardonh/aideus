...
def test_aideus_provider_subclass_exists():
    """AideusMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _AIDEUS_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _AIDEUS_PROVIDER_CLS is not None
    assert issubclass(_AIDEUS_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_aideus_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AideusMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _AIDEUS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("AIDEUS_HOME", str(tmp_path))
    ...
    assert _AIDEUS_PROVIDER_CLS is not None
    assert isinstance(provider, _AIDEUS_PROVIDER_CLS)
    assert provider._aideus_server_name == "srv"
