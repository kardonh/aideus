...
def _clean_state():
    ...
    saved = {}
    for k in ("AIDEUS_INTERACTIVE", "AIDEUS_GATEWAY_SESSION", "AIDEUS_EXEC_ASK", "AIDEUS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["AIDEUS_INTERACTIVE"] = "1"
        session_key = os.getenv("AIDEUS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No AIDEUS_INTERACTIVE or AIDEUS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
