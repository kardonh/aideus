...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aideus_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_aideus_bin(self):
        """After _INSTALL_FAILED, manual install in AIDEUS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        aideus_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(aideus_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(aideus_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aideus_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == aideus_bin
            assert _tirith_mod._resolved_path == aideus_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# AIDEUS_HOME isolation
# ---------------------------------------------------------------------------

class TestAideusHomeIsolation:
    def test_aideus_bin_dir_respects_aideus_home(self):
        """_aideus_bin_dir must use AIDEUS_HOME, not hardcoded ~/.aideus."""
        from tools.tirith_security import _aideus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AIDEUS_HOME": tmpdir}):
            result = _aideus_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_aideus_home(self):
        """_failure_marker_path must use AIDEUS_HOME, not hardcoded ~/.aideus."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AIDEUS_HOME": "/custom/aideus"}):
            result = _failure_marker_path()
        assert result == "/custom/aideus/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets AIDEUS_HOME; verify it's active."""
        aideus_home = os.getenv("AIDEUS_HOME")
        assert aideus_home is not None, "AIDEUS_HOME should be set by conftest"
        assert "aideus_test" in aideus_home, "Should point to test temp dir"

    def test_get_aideus_home_fallback(self):
        """Without AIDEUS_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_aideus_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AIDEUS_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".aideus")
            result = _get_aideus_home()
