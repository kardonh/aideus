...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure AIDEUS_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".aideus").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", aideus_home=str(tmp_path / ".aideus"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", aideus_home=profile_home)
        assert p._client.project == "aideus-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".aideus" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        aideus_home = tmp_path / ".aideus"
        aideus_home.mkdir(exist_ok=True)
        monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", aideus_home=str(aideus_home))
        return p
