"""
Tests for mcp_serve — Aideus MCP server.
...
def _isolate_aideus_home(tmp_path, monkeypatch):
    """Redirect AIDEUS_HOME to a temp directory."""
    monkeypatch.setenv("AIDEUS_HOME", str(tmp_path))
    try:
        import aideus_constants
        monkeypatch.setattr(aideus_constants, "get_aideus_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking aideus_state schema."""
