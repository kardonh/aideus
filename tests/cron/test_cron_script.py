...
- Path resolution (absolute, relative to AIDEUS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp AIDEUS_HOME."""
    aideus_home = tmp_path / ".aideus"
    aideus_home.mkdir()
    (aideus_home / "cron").mkdir()
    (aideus_home / "cron" / "output").mkdir()
    (aideus_home / "scripts").mkdir()
    monkeypatch.setenv("AIDEUS_HOME", str(aideus_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "AIDEUS_DIR", aideus_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", aideus_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", aideus_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", aideus_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.aideus/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("AIDEUS_SESSION_PLATFORM") is None
        assert os.environ.get("AIDEUS_SESSION_CHAT_ID") is None
        assert os.environ.get("AIDEUS_SESSION_CHAT_NAME") is None
