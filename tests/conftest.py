"""Shared fixtures for the aideus-agent test suite.
...
2. **Isolated AIDEUS_HOME.** AIDEUS_HOME points to a per-test tempdir so
   code reading ``~/.aideus/*`` via ``get_aideus_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".aideus"`` instead of the canonical
   ``get_aideus_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No AIDEUS_SESSION_* inheritance** — the agent's current gateway
...
# AIDEUS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_AIDEUS_BEHAVIORAL_VARS = frozenset({
    "AIDEUS_YOLO_MODE",
    "AIDEUS_INTERACTIVE",
    "AIDEUS_QUIET",
    ...
    "AIDEUS_SESSION_PLATFORM",
    "AIDEUS_SESSION_CHAT_ID",
    "AIDEUS_SESSION_CHAT_NAME",
    "AIDEUS_SESSION_THREAD_ID",
    "AIDEUS_SESSION_SOURCE",
    "AIDEUS_SESSION_KEY",
    "AIDEUS_GATEWAY_SESSION",
    "AIDEUS_PLATFORM",
    "AIDEUS_MODEL",
    "AIDEUS_INFERENCE_MODEL",
    "AIDEUS_INFERENCE_PROVIDER",
    "AIDEUS_TUI_PROVIDER",
    "AIDEUS_MANAGED",
    "AIDEUS_DEV",
    "AIDEUS_CONTAINER",
    ...
    "AIDEUS_TIMEZONE",
    "AIDEUS_REDACT_SECRETS",
    "AIDEUS_BACKGROUND_NOTIFICATIONS",
    "AIDEUS_EXEC_ASK",
    "AIDEUS_HOME_MODE",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and AIDEUS_HOME to per-test tempdirs so code that
    reads ``~/.aideus/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral AIDEUS_* vars that could change test semantics.
    for name in _AIDEUS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect AIDEUS_HOME to a per-test tempdir. Code that reads
    #    ``~/.aideus/*`` via ``get_aideus_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.aideus/*`` via
    #    ``Path.home() / ".aideus"`` instead of ``get_aideus_home()``
    #    is a bug to fix at the callsite.
    fake_aideus_home = tmp_path / "aideus_test"
    fake_aideus_home.mkdir()
    (fake_aideus_home / "sessions").mkdir()
    (fake_aideus_home / "cron").mkdir()
    (fake_aideus_home / "memories").mkdir()
    (fake_aideus_home / "skills").mkdir()
    monkeypatch.setenv("AIDEUS_HOME", str(fake_aideus_home))
    ...
    #    ~/.aideus/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import aideus_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_aideus_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "aideus_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal aideus config dict suitable for unit tests."""
    return {
