...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Aideus-Session-Id header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists aideus-agent as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to aideus-agent
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_aideus_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
def _derive_chat_session_id(
    ...
    the same Aideus session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through aideus-agent's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``aideus sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the aideus-api-server default.
        """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "aideus-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return aideus-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Aideus version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "features": {
                ...
                "session_continuity_header": "X-Aideus-Session-Id",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-Aideus-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Aideus-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Aideus-Session-Id rejected: "
            ...
            # conversation map to the same Aideus session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``aideus.tool.progress`` with ``status: running``.
        ...
        return web.json_response(response_data, headers={"X-Aideus-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Aideus-Session-Id"] = session_id
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: aideus.tool.progress`` SSE event so
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from aideus_cli.auth import has_usable_secret
