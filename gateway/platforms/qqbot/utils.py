...
def _get_aideus_version() -> str:
    """Return the aideus-agent package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("aideus-agent")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Aideus/<aideus_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Aideus/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    aideus_version = _get_aideus_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Aideus/{aideus_version})"
