#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Aideus Skills Hub.
...
Used by aideus_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            aideus_meta = metadata.get("aideus", {})
            if isinstance(aideus_meta, dict):
                tags = aideus_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                aideus_meta = meta_block.get("aideus", {})
                if isinstance(aideus_meta, dict):
                    tags = aideus_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Aideus centralized index source
# ---------------------------------------------------------------------------

AIDEUS_INDEX_URL = "https://aideus-agent.nousresearch.com/docs/api/skills-index.json"
AIDEUS_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "aideus-index.json"
AIDEUS_INDEX_TTL = 6 * 3600  # 6 hours


def _load_aideus_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for AIDEUS_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if AIDEUS_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - AIDEUS_INDEX_CACHE_FILE.stat().st_mtime
            if age < AIDEUS_INDEX_TTL:
                return json.loads(AIDEUS_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(AIDEUS_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Aideus index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Aideus index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        AIDEUS_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        AIDEUS_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class AideusIndexSource(SkillSource):
    """Skill source backed by the centralized Aideus Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_aideus_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "aideus-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "aideus-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        AideusIndexSource(auth=auth), # Centralized index (search + resolved install paths)
