...
One ``CDPSupervisor`` runs per Aideus ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "aideus-dialog-bridge.invalid"
...
(() => {
  if (window.__aideusDialogBridgeInstalled) return;
  window.__aideusDialogBridgeInstalled = true;
  const ENDPOINT = "http://aideus-dialog-bridge.invalid/";
