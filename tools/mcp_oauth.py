...
This module provides the glue:
    - ``AideusTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Aideus Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses AIDEUS_HOME so each profile gets its own OAuth tokens.
    Layout: ``AIDEUS_HOME/mcp-tokens/``
    """
    try:
        from aideus_constants import get_aideus_home
        base = Path(get_aideus_home())
    except ImportError:
        base = Path(os.environ.get("AIDEUS_HOME", str(Path.home() / ".aideus")))
...
# ---------------------------------------------------------------------------
# AideusTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AideusTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        AIDEUS_HOME/mcp-tokens/<server_name>.json         -- tokens
        AIDEUS_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Aideus records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Aideus.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Aideus Agent")
    scope = cfg.get("scope")
