...
it is automatically downloaded from GitHub releases to $AIDEUS_HOME/bin/tirith.
...
def _get_aideus_home() -> str:
    """Return the Aideus home directory, respecting AIDEUS_HOME env var."""
    return str(get_aideus_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_aideus_home(), ".tirith-install-failed")
...
def _aideus_bin_dir() -> str:
    """Return $AIDEUS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_aideus_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $AIDEUS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_aideus_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $AIDEUS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $AIDEUS_HOME/bin/tirith
    ...
    aideus_bin = os.path.join(_aideus_bin_dir(), "tirith")
    if os.path.isfile(aideus_bin) and os.access(aideus_bin, os.X_OK):
        _resolved_path = aideus_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return aideus_bin
