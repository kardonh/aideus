...
`mcp_tool.py`, and `aideus_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AideusMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_aideus_provider_class() -> Optional[type]:
    ...
    class AideusMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._aideus_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AideusTokenStorage` persisting an absolute
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._aideus_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._aideus_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return AideusMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_AIDEUS_PROVIDER_CLS: Optional[type] = _make_aideus_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AideusMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _AIDEUS_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = AideusTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``aideus mcp remove <name>`` and (indirectly) by
        ``aideus mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
