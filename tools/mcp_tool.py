...
discovers their tools, and registers them into the aideus-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.aideus/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.aideus/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            aideus_home = os.path.expanduser(
                os.getenv(
                    "AIDEUS_HOME", os.path.join(os.path.expanduser("~"), ".aideus")
                )
            )
            candidates = [
                os.path.join(aideus_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `aideus mcp login {server_name}` (or delete the tokens "
            f"file under ~/.aideus/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Aideus config file.
    ...
    ``os.environ`` (which includes ``~/.aideus/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Aideus's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Aideus registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``aideus tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Aideus registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple aideus
