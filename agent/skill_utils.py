...
from aideus_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``AIDEUS_PLATFORM`` or
            ``AIDEUS_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.aideus/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against AIDEUS_HOME, not cwd
        if not p.is_absolute():
            p = (aideus_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.aideus/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    aideus = metadata.get("aideus") or {}
    if not isinstance(aideus, dict):
        aideus = {}
    return {
        "fallback_for_toolsets": aideus.get("fallback_for_toolsets", []),
        "requires_toolsets": aideus.get("requires_toolsets", []),
        "fallback_for_tools": aideus.get("fallback_for_tools", []),
        "requires_tools": aideus.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    aideus = metadata.get("aideus")
    if not isinstance(aideus, dict):
        return []
    raw = aideus.get("config")
    if not raw:
        return []
