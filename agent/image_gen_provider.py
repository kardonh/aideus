...
as ``kind: backend``) or ``~/.aideus/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``aideus tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``aideus tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``aideus tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$AIDEUS_HOME/cache/images/``, creating parents as needed."""
    from aideus_constants import get_aideus_home

    path = get_aideus_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$AIDEUS_HOME/cache/images/``.
