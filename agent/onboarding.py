...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Aideus starts and finds ``~/.openclaw/``.

    Points users at ``aideus claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``aideus claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Aideus, run "
        "`aideus claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `aideus claw cleanup` (renames it to ~/.openclaw.pre-migration — "
