"""Anthropic Messages API adapter for Aideus Agent.

Translates between Aideus's internal OpenAI-style message format and
...
# Aideus effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Aideus historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Aideus)
    ...
    # 1. Aideus-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Aideus configs.
...
# ── Aideus-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.aideus/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_AIDEUS_OAUTH_FILE = get_aideus_home() / ".anthropic_oauth.json"
...
def run_aideus_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Aideus-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Aideus with your Claude Pro/Max subscription.")
    print()
...
def read_aideus_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Aideus-managed OAuth credentials from ~/.aideus/.anthropic_oauth.json."""
    if _AIDEUS_OAUTH_FILE.exists():
        try:
            data = json.loads(_AIDEUS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Aideus OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See aideus-agent#13848.
            #
    ...
    # are preserved.  See aideus-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Aideus Agent", "Claude Code")
                text = text.replace("Aideus agent", "Claude Code")
                text = text.replace("aideus-agent", "claude-code")
    ...
    # silently hides reasoning text that Aideus surfaces in its CLI. We
