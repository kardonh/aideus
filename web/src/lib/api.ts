...
const SESSION_HEADER = "X-Aideus-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__AIDEUS_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Aideus dashboard server");
}
...
export const api = {
  ...
  updateAideus: () =>
    fetchJSON<ActionResponse>("/api/aideus/update", { method: "POST" }),
