...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      ...
      aideusVenv = aideus-agent.aideusVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "aideus-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${aideusVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "aideus-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "aideus-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${aideus-agent}/bin/aideus || (echo "FAIL: aideus binary missing"; exit 1)
          test -x ${aideus-agent}/bin/aideus-agent || (echo "FAIL: aideus-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${aideus-agent}/bin/aideus version 2>&1 | grep -qi "aideus" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "aideus-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in aideus aideus-agent aideus-acp; do
            test -x ${aideus-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "aideus-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking aideus --help ==="
          ${aideus-agent}/bin/aideus --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${aideus-agent}/bin/aideus --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "aideus-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${aideus-agent}/share/aideus-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${aideus-agent}/share/aideus-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "AIDEUS_BUNDLED_SKILLS" ${aideus-agent}/bin/aideus || \
            (echo "FAIL: AIDEUS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: AIDEUS_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "aideus-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${aideus-agent}/share/aideus-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${aideus-agent}/share/aideus-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "AIDEUS_BUNDLED_PLUGINS" ${aideus-agent}/bin/aideus || \
            (echo "FAIL: AIDEUS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: AIDEUS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "aideus-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${aideus-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${aideus-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${aideus-agent}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "AIDEUS_TUI_DIR" ${aideus-agent}/bin/aideus || \
            (echo "FAIL: AIDEUS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: AIDEUS_TUI_DIR set in wrapper"
        ...
        # Verify AIDEUS_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        aideus-node = pkgs.runCommand "aideus-node-version" { } ''
          set -e
          echo "=== Checking AIDEUS_NODE in wrapper ==="
          grep -q "AIDEUS_NODE" ${aideus-agent}/bin/aideus || \
            (echo "FAIL: AIDEUS_NODE not set in wrapper"; exit 1)
          echo "PASS: AIDEUS_NODE present in wrapper"

          AIDEUS_NODE=$(sed -n "s/^export AIDEUS_NODE='\(.*\)'/\1/p" ${aideus-agent}/bin/aideus)
          test -x "$AIDEUS_NODE" || (echo "FAIL: AIDEUS_NODE=$AIDEUS_NODE not executable"; exit 1)
          echo "PASS: AIDEUS_NODE executable at $AIDEUS_NODE"

          NODE_MAJOR=$("$AIDEUS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All AIDEUS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify AIDEUS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "aideus-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(AIDEUS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking AIDEUS_MANAGED guards ==="
          check_blocked "config set" ${aideus-agent}/bin/aideus config set model foo
          check_blocked "config edit" ${aideus-agent}/bin/aideus config edit
        ...
        extra-python-packages = let
          ...
          aideusWithExtra = aideus-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "aideus-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${aideusWithExtra}/bin/aideus || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${aideusWithExtra}/bin/aideus || \
          ...
          if grep -q "PYTHONPATH" ${aideus-agent}/bin/aideus; then
        ...
        in pkgs.runCommand "aideus-config-roundtrip" {
          ...
          merge_and_load() {
            local aideus_home="$1"
            export AIDEUS_HOME="$aideus_home"
            ${configMergeScript} ${nixSettings} "$aideus_home/config.yaml"
            ${aideusVenv}/bin/python3 -c '
import json, sys
