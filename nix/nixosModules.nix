# nix/nixosModules.nix — NixOS module for aideus-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: aideus runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $AIDEUS_HOME/.env
# and read by aideus at startup — no container recreation needed for env changes.
#
# Tool resolution: the aideus wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.aideus-agent;
    effectivePackage = if cfg.extraPythonPackages == [ ] then cfg.package
      else cfg.package.override { inherit (cfg) extraPythonPackages; };
    aideus-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-aideus-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "aideus-config-attrs";
      description = "Aideus YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "aideus-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "aideus-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'AIDEUS_DOC_EOF'\n${value}\nAIDEUS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "aideus-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/aideus";
    ...
    # aideus user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "aideus-container-entrypoint" ''
      set -eu

      AIDEUS_UID="''${AIDEUS_UID:?AIDEUS_UID must be set}"
      AIDEUS_GID="''${AIDEUS_GID:?AIDEUS_GID must be set}"

      # ── Group: ensure a group with GID=$AIDEUS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$AIDEUS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="aideus"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$AIDEUS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$AIDEUS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$AIDEUS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$AIDEUS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="aideus"
        TARGET_HOME="/home/aideus"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$AIDEUS_UID" -g "$AIDEUS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$AIDEUS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$AIDEUS_UID:$AIDEUS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure AIDEUS_HOME is owned by the target user
      if [ -n "''${AIDEUS_HOME:-}" ] && [ -d "$AIDEUS_HOME" ]; then
        chown -R "$AIDEUS_UID:$AIDEUS_GID" "$AIDEUS_HOME"
      fi
      ...
      if [ ! -f /var/lib/aideus-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/aideus-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/aideus ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/aideus
        chmod 0440 /etc/sudoers.d/aideus
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$AIDEUS_UID" --regid="$AIDEUS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $AIDEUS_HOME/.env.
    ...
    # Default: /var/lib/aideus/workspace → /data/workspace.
  ...
  in {
    options.services.aideus-agent = with lib; {
      enable = mkEnableOption "Aideus Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = aideus-agent;
        description = "The aideus-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/aideus";
        description = "State directory. Contains .aideus/ subdir (AIDEUS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Aideus config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $AIDEUS_HOME/.env at activation time.
          Aideus reads this file on every startup via load_aideus_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $AIDEUS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $AIDEUS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `aideus gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the aideus user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the aideus plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Aideus discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          aideus_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as aideus (python312).
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the aideus CLI to environment.systemPackages and export
          AIDEUS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.aideus symlink to the service
            stateDir. These users are automatically added to the aideus group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.aideus-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the aideus CLI to system PATH and export AIDEUS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.aideus/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.AIDEUS_HOME = "${cfg.stateDir}/.aideus";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.aideus-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the aideus user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.aideus-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed aideus
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure aideus is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.aideus        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aideus/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aideus/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aideus/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aideus/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aideus/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."aideus-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.aideus
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.aideus ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.aideus ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.aideus -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.aideus/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.aideus/$_subdir"
            chmod 2770 "${cfg.stateDir}/.aideus/$_subdir"
            find "${cfg.stateDir}/.aideus/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.aideus/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.aideus/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aideus/config.yaml
            chmod 0640 ${cfg.stateDir}/.aideus/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.aideus/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aideus/.managed
          chmod 0644 ${cfg.stateDir}/.aideus/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.aideus/.container-mode <<'AIDEUS_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    aideus_bin=${containerDataDir}/current-package/bin/aideus
    AIDEUS_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aideus/.container-mode
            chmod 0644 ${cfg.stateDir}/.aideus/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.aideus/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.aideus";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.aideus" ]; then
                  rm -f "${symlinkPath}"
                  echo "aideus-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.aideus -> stateDir/.aideus for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.aideus";
                target = "${cfg.stateDir}/.aideus";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "aideus-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.aideus/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.aideus/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.aideus/auth.json
              fi
            ''}
          ''}
          ...
          # Aideus reads $AIDEUS_HOME/.env at startup via load_aideus_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.aideus/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'AIDEUS_NIX_ENV_EOF'
    ${envFileContent}
    AIDEUS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.aideus/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.aideus/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aideus/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.aideus-agent = {
          description = "Aideus Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            AIDEUS_HOME = "${cfg.stateDir}/.aideus";
            AIDEUS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $AIDEUS_HOME/.env by the activation script. load_aideus_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/aideus"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the aideus group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.aideus-agent = {
          description = "Aideus Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              AIDEUS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              AIDEUS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env AIDEUS_UID="$AIDEUS_UID" \
                --env AIDEUS_GID="$AIDEUS_GID" \
                --env AIDEUS_HOME=${containerDataDir}/.aideus \
                --env AIDEUS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/aideus gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
